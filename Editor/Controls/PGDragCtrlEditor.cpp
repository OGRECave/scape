...
DragButton::DragButton() : mIsDragging(false), mScrollDistance(0), mLastMousePosition(0, 0)
{
}
...
void DragButton::onMouseMotion(wxMouseEvent& event)
{
    if (mIsDragging)
    {
        ...
        if (deltaY == 0)
        {
            if (mousePosition.y <= displayRect.GetTop() + 1)
                --deltaY;
            else if (mousePosition.y >= displayRect.GetBottom() - +1)
                ++deltaY;
        }
...
// ----------------------------------------------------------------------------
wxWindow* PGDragCtrlEditor::CreateControls(
    wxPropertyGrid* propgrid, wxPGProperty* property, const wxPoint& pos, const wxSize& sz, wxWindow** pSecondary) const
{
    const int margin = 1;
    ...
    propgrid->Connect(wxPG_SUBID2, wxEVT_SCROLL_CHANGED,
        (wxObjectEventFunction)(wxEventFunction)(wxCommandEventFunction)&wxPropertyGrid::OnCustomEditorEvent, NULL,
        propgrid);
...
// ----------------------------------------------------------------------------
bool PGDragCtrlEditor::OnEvent(wxPropertyGrid* propgrid, wxPGProperty* property, wxWindow* wnd, wxEvent& event) const
{
