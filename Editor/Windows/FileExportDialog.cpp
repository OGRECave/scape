...
// ----------------------------------------------------------------------------
FileExportDialog::FileExportDialog(wxWindow* parentWindow, wxWindowID windowId, const FileExportItemList& encoders,
    const string& selectedEncoder, bool flipX, bool flipY)
    : wxDialog(
          parentWindow, windowId, _T("File export settings"), wxDefaultPosition, wxDefaultSize, wxDEFAULT_DIALOG_STYLE)
    , mEncoders(encoders)
{
    ...
    // list
    mListBox
        = new wxListBox(this, wxID_ANY, wxDefaultPosition, wxDefaultSize, encoderArray, wxLB_SINGLE | wxLB_ALWAYS_SB);

    topSizer->Add(mListBox, wxSizerFlags().Expand().Proportion(1).TripleBorder(wxLEFT | wxRIGHT));

    // list item description
    mDescriptionTextCtrl = new wxTextCtrl(this, wxID_ANY, Utils::emptyString, wxDefaultPosition, wxDefaultSize,
        // wxSIMPLE_BORDER  |
