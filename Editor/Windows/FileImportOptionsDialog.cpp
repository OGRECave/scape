...
// ----------------------------------------------------------------------------
FileImportOptionsDialog::FileImportOptionsDialog(wxWindow* parentWindow, wxWindowID windowId, const wxString& caption, const wxString& fileName, size_t fileSize, const FileImportOptionsDialogItemMap& itemMap, const StringStringMap& valueMap)
   ...
   mDirtyValueMap(false)
{
	...
	if (showWidth || showHeight)
	{
		...
		if (showWidth)
		{
			...
			mWidthTextCtrl = new wxTextCtrl(this, wxID_ANY, valueWidth, wxDefaultPosition, wxDefaultSize, 0, integerValidator);
		...
		if (showHeight)
		{
			...
			mHeightTextCtrl = new wxTextCtrl(this, wxID_ANY, valueHeight, wxDefaultPosition, wxDefaultSize, 0, integerValidator);
	...
	if (showBpp)
	{
		wxStaticBoxSizer* categorySizer = new wxStaticBoxSizer(wxHORIZONTAL, this, _T("Bits per pixel (bpp)"));
	...
	if (showEndian)
	{
		...
		mBigEndianButton = new wxRadioButton(
			this, 
			wxID_ANY, 
			_T("Big Endian (MAC)"),
			wxDefaultPosition, 
		...
		if (valueEndian) mBigEndianButton->SetValue(true); else button->SetValue(true);
...
const StringStringMap& FileImportOptionsDialog::getValueMap()
{
	if (mDirtyValueMap)
	{
		if (mFlipXCheckBox) mValueMap[PROPERTY_FLIPX] = wxString::Format("%d", mFlipXCheckBox->GetValue());
		if (mFlipYCheckBox) mValueMap[PROPERTY_FLIPY] = wxString::Format("%d", mFlipYCheckBox->GetValue());
		if (mWidthTextCtrl) mValueMap[PROPERTY_WIDTH] = mWidthTextCtrl->GetValue();
		if (mHeightTextCtrl) mValueMap[PROPERTY_HEIGHT] = mHeightTextCtrl->GetValue();
		if (mBigEndianButton) mValueMap[PROPERTY_BIGENDIAN] = wxString::Format("%d", mBigEndianButton->GetValue());

		size_t bppIndex = 0;
		std::list<wxRadioButton*>::iterator bppIt, bppItEnd = mBppButtons.end();
		for (bppIt = mBppButtons.begin(); bppIt != bppItEnd; ++bppIt, ++bppIndex)
		{
			if ((*bppIt)->GetValue()) break;
		}
		if (bppIndex < mBppButtons.size()) mValueMap[PROPERTY_BPP] = wxString::Format(_T("%d"), bppIndex);
