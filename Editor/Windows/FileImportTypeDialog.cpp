...
// ----------------------------------------------------------------------------
FileImportTypeDialog::FileImportTypeDialog(wxWindow* parentWindow, wxWindowID windowId,
                                           const StringList& decoders, const string& selectedEncoder)
    : wxDialog(parentWindow, windowId, _T("File export settings"), wxDefaultPosition, wxDefaultSize,
      ...
      mEncoders(decoders)
{
    ...
    // list
    mListBox = new wxListBox(this, wxID_ANY, wxDefaultPosition, wxDefaultSize, decoderArray,
                             wxLB_SINGLE | wxLB_ALWAYS_SB);

    topSizer->Add(mListBox, wxSizerFlags().Expand().Proportion(1).TripleBorder(wxLEFT | wxRIGHT));

    // list item description
    mDescriptionTextCtrl =
        new wxTextCtrl(this, wxID_ANY, Utils::emptyString, wxDefaultPosition, wxDefaultSize,
                       // wxSIMPLE_BORDER  |
                       wxTE_MULTILINE | wxTE_READONLY | wxTE_BESTWRAP);

    mDescriptionTextCtrl->SetBackgroundColour(GetBackgroundColour());

    topSizer->Add(mDescriptionTextCtrl,
                  wxSizerFlags().Expand().Proportion(0).TripleBorder(wxLEFT | wxRIGHT));
...
// ----------------------------------------------------------------------------
void FileImportTypeDialog::onListBoxDClick(wxCommandEvent& event) { EndModal(wxID_OK); }
