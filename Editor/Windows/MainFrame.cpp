...
MainFrame::MainFrame(Application* application)
    : wxFrame(NULL, wxID_ANY, APPLICATION_TITLE, wxDefaultPosition, wxSize(1240, 885),
      ...
      mApplication(application), mAuiManager(NULL), mRenderPanel(NULL), mAttachedInputToEngine(false),
      mEngineInterface(NULL), mPropertyGrid(NULL), mTimer(NULL), mMainToolBar(NULL), mPresetPanel(NULL),
      ...
      mSelectedToolElementGroupId((ScapeEngine::EScapeUIElementGroupId)-1)
{
    ...
    mPropertyGrid =
        new UIElementPropertyGrid(this, wxT("properties"), wxT("Properties"), this, mAuiManager);
...
// ----------------------------------------------------------------------------
void MainFrame::onMenuExit(wxCommandEvent& WXUNUSED(event)) { Close(true); }
...
void MainFrame::onMenuImportImageFile(wxCommandEvent& event)
{
    ...
    for (fileFilterIt = fileFilterMap.begin(); fileFilterIt != fileFilterItEnd; ++fileFilterIt)
    {
        ...
        fileFilters += wxString::Format(_T("%s (%s)|%s|"), fileFilterLabel.c_str(),
                                        fileFilterIt->second.second.c_str(),
                                        wxString(fileFilterIt->second.second).Lower().c_str());
    }
    fileFilters += wxString::Format(_T("All files (%s)|%s"), wxFileSelectorDefaultWildcardStr,
    ...
    wxString fullPath =
        wxFileSelector(_T("Choose heightfield image to import"), lastPath.c_str(), wxEmptyString,
    ...
    if (fullPath.length())
    {
        ...
        StringList decoderDescs =
            mEngineInterface->getUIElementNameList(ScapeEngine::SCAPEUIELEMENTGROUPID_FILEIMPORT);

        // Use FileImportType dialog if extension wasn't recognized by the decoders to let the user choose a
        // decoder manually
        ...
        if (decoderName.length())
        {
            ...
            for (nameIt = nameList.begin(); nameIt != nameItEnd; ++nameIt)
            {
                ...
                item.label = mEngineInterface->getUIElementPropertyField(decoderName, *nameIt,
                                                                         UIELEMENTPROPERTYNAME_SHORT);
                item.description = mEngineInterface->getUIElementPropertyField(decoderName, *nameIt,
                                                                               UIELEMENTPROPERTYNAME_LONG);
                item.type = mEngineInterface->getUIElementPropertyField(decoderName, *nameIt,
                                                                        UIELEMENTPROPERTYNAME_TYPE);
                item.options = mEngineInterface->getUIElementPropertyField(decoderName, *nameIt,
                                                                           UIELEMENTPROPERTYNAME_OPTIONS);
                itemMap[*nameIt] = item;
            }
            ...
            FileImportOptionsDialog dialog(this, wxID_ANY, dialogCaption, fileName, fileSize, itemMap,
                                           valueMap);
...
{
    StringList names =
        mEngineInterface->getUIElementNameList(ScapeEngine::SCAPEUIELEMENTGROUPID_FILEEXPORT);
    ...
    for (nameIt = names.begin(); nameIt != nameItEnd; ++nameIt)
    {
        FileExportItem item;
        item.name = *nameIt;
        item.label = mEngineInterface->getUIElementPropertyField(*nameIt, UIELEMENTPROPERTYNAME_LABEL,
                                                                 UIELEMENTPROPERTYNAME_SHORT);
        item.description = mEngineInterface->getUIElementPropertyField(*nameIt, UIELEMENTPROPERTYNAME_LABEL,
                                                                       UIELEMENTPROPERTYNAME_LONG);
    ...
    if (dialog.ShowModal() == wxID_OK)
    {
        ...
        if (selectedEncoder.length())
        {
            ...
            wxString fullPath = wxFileSelector(
                _T("Export File"), lastPath, lastName, wxEmptyString,
            ...
            if (fullPath.length())
            {
                ...
                mEngineInterface->setUIElementPropertyValue(ScapeEngine::SCAPEUIELEMENTGROUPID_FILEEXPORT,
                                                            selectedEncoder, _T("FLIPX"),
                                                            string(wxString::Format(_T("%d"), flipX)));
                mEngineInterface->setUIElementPropertyValue(ScapeEngine::SCAPEUIELEMENTGROUPID_FILEEXPORT,
                                                            selectedEncoder, _T("FLIPY"),
                                                            string(wxString::Format(_T("%d"), flipY)));
                string error;
...
// ----------------------------------------------------------------------------
void MainFrame::onToolBarItemClicked(const wxString& toolBarName, const wxString& itemName,
                                     int itemCategory)
{
...
// ----------------------------------------------------------------------------
wxString MainFrame::onUpdatePropertyGridPropertyValue(const wxString& gridIdName,
                                                      const wxString& propertyName, const wxString& value)
{
...
// ----------------------------------------------------------------------------
wxString MainFrame::onResetPropertyGridPropertyValue(const wxString& gridIdName,
                                                     const wxString& propertyName)
{
    string value = mEngineInterface->getUIElementPresetPropertyValue(
        mSelectedToolElementGroupId, mSelectedToolElementName, Utils::defaultNameString,
        string(propertyName));
...
{
    mPresetToolSelectedItemMap[std::pair<int, string>(mSelectedToolElementGroupId,
                                                      mSelectedToolElementName)] = presetName;
}
...
{
    StringStringMap valueMap = mEngineInterface->getUIElementPropertyValueMap(mSelectedToolElementGroupId,
...
{
    mEngineInterface->deleteUIElementPreset(mSelectedToolElementGroupId, mSelectedToolElementName,
                                            string(presetName));
    populatePresetPanel();
}

// ----------------------------------------------------------------------------
void MainFrame::onPresetPanelChangePresetName(const wxString& presetPanelName,
                                              const wxString& oldPresetName, const wxString& newPresetName)
{
    StringStringMap valueMap = mEngineInterface->getUIElementPresetPropertyValueMap(
...
{
    string presetName = mEngineInterface->importUIElementPreset(mSelectedToolElementGroupId,
                                                                mSelectedToolElementName, string(fileName));
...
{
    mEngineInterface->exportUIElementPreset(mSelectedToolElementGroupId, mSelectedToolElementName,
                                            string(fileName));
}
...
void MainFrame::populatePropertyGrid()
{
    if (mPropertyGrid)
    {
        StringList nameList = mEngineInterface->getUIElementPropertyNameList(mSelectedToolElementGroupId,
                                                                             mSelectedToolElementName);
        ...
        for (nameIt = nameList.begin(); nameIt != nameItEnd; ++nameIt)
        {
            ...
            item.name = *nameIt;
            item.label = mEngineInterface->getUIElementPropertyField(propertySetName, *nameIt,
                                                                     UIELEMENTPROPERTYNAME_SHORT);
            item.description = mEngineInterface->getUIElementPropertyField(propertySetName, *nameIt,
                                                                           UIELEMENTPROPERTYNAME_LONG);
            item.category = mEngineInterface->getUIElementPropertyField(propertySetName, *nameIt,
                                                                        UIELEMENTPROPERTYNAME_CATEGORY);
            item.type = mEngineInterface->getUIElementPropertyField(propertySetName, *nameIt,
                                                                    UIELEMENTPROPERTYNAME_TYPE);
        ...
        mPropertyGrid->setValues(mEngineInterface->getUIElementPropertyValueMap(mSelectedToolElementGroupId,
                                                                                mSelectedToolElementName));
    }
}
...
void MainFrame::populatePresetPanel(const string& selectedPreset, bool edit)
{
    if (mPresetPanel)
    {
        ...
        mPresetPanel->populate(mEngineInterface->getUIElementPresetPropertyNames(
            mSelectedToolElementGroupId, mSelectedToolElementName));
...
void MainFrame::populateMainToolBar()
{
    ...
    {
        StringList nameList =
            mEngineInterface->getUIElementNameList(ScapeEngine::SCAPEUIELEMENTGROUPID_OPERATION);
        ...
        for (nameIt = nameList.begin(); nameIt != nameItEnd; ++nameIt)
        {
            ...
            item.name = *nameIt;
            item.label = mEngineInterface->getUIElementPropertyField(*nameIt, _T("LABEL"),
                                                                     UIELEMENTPROPERTYNAME_SHORT);
            item.description = mEngineInterface->getUIElementPropertyField(*nameIt, _T("LABEL"),
                                                                           UIELEMENTPROPERTYNAME_LONG);
            item.icon = mEngineInterface->getUIElementPropertyField(*nameIt, _T("LABEL"),
                                                                    UIELEMENTPROPERTYNAME_ICON);
