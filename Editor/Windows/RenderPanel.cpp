...
// ----------------------------------------------------------------------------
RenderPanel::RenderPanel(int viewId, wxWindow* parent, ScapeEngine::EngineInterface* engineInterface,
                         wxAuiManager* auiManager)
    : wxPanel(parent, wxID_ANY), mViewId(viewId), mHasView(false), mHasFocus(false), mViewRect(0, 0, 0, 0),
      mPreviousViewRect(0, 0, 0, 0), mEngineInterface(engineInterface), mAuiManager(auiManager)
{
...
// ----------------------------------------------------------------------------
void RenderPanel::onSize(wxSizeEvent& event) { updateRect(); }

// ----------------------------------------------------------------------------
void RenderPanel::onMove(wxMoveEvent& event) { updateRect(); }
    ...
    if (mViewRect != mPreviousViewRect)
    {
        if (mViewRect.width >= MINIMAL_SIZE_RENDERVIEW && mViewRect.height >= MINIMAL_SIZE_RENDERVIEW)
        {
            if (!mHasView)
            {
                ...
                mEngineInterface->createRenderView(mViewId, windowHandle.c_str(), mViewRect.x, mViewRect.y,
                                                   mViewRect.width, mViewRect.height);
                mHasView = true;
                if (mHasFocus)
                {
                    wxFocusEvent event;
            ...
            {
                mEngineInterface->onRenderViewMovedOrResized(mViewId, mViewRect.x, mViewRect.y,
                                                             mViewRect.width, mViewRect.height);
...
// ----------------------------------------------------------------------------
void RenderPanel::onClick(wxMouseEvent& event) { SetFocus(); }
