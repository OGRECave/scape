...
// ----------------------------------------------------------------------------
UIElementLayerPanel::UIElementLayerPanel(wxWindow* parentWindow, const wxString& name, const wxString& caption,
    UIElementLayerPanelListener* listener, wxAuiManager* auiManager)
...
void UIElementLayerPanel::onItemSelected(wxListEvent& event)
{
    ...
    bool enable
        = true; //(mListCtrl->GetItemText(mSelectedLayerIndex).compare(wxString(Utils::defaultNameString)) != 0);
...
void UIElementLayerPanel::selectLayer(int layerIndex, bool edit)
{
    if (isValidIndex(layerIndex))
    {
        mListCtrl->SetItemState(
            layerIndex, wxLIST_STATE_FOCUSED | wxLIST_STATE_SELECTED, wxLIST_STATE_FOCUSED | wxLIST_STATE_SELECTED);
...
void UIElementLayerPanel::onImportCommand(wxCommandEvent& event)
{
    ...
    wxString path = wxFileSelector(_T("Choose layer file to import"), lastPath.c_str(), wxEmptyString, wxEmptyString,
        wxString::Format(_T("Layer files (*.spe)|*.spe|All files (%s)|%s"), wxFileSelectorDefaultWildcardStr,
...
void UIElementLayerPanel::onExportCommand(wxCommandEvent& event)
{
    ...
    wxString path = wxFileSelector(_T("Choose filename to export layer"), lastPath.c_str(), wxEmptyString,
        wxEmptyString, wxString::Format(_T("Layer files (*.spe)|*.spe|All files (%s)|%s"),
