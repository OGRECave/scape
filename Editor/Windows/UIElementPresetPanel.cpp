...
// ----------------------------------------------------------------------------
UIElementPresetPanel::UIElementPresetPanel(wxWindow* parentWindow, const wxString& name, const wxString& caption,
    UIElementPresetPanelListener* listener, wxAuiManager* auiManager)
    : wxPanel(parentWindow)
    , mParentWindow(parentWindow)
    , mPresetPanelName(name)
...
void UIElementPresetPanel::onItemSelected(wxListEvent& event)
{
    ...
    if (mListener)
    {
        mListener->onPresetPanelSelectPreset(mPresetPanelName, mListCtrl->GetItemText(mSelectedPresetIndex));
    }

    bool enable
        = true; //(mListCtrl->GetItemText(mSelectedPresetIndex).compare(wxString(Utils::defaultNameString)) != 0);
...
void UIElementPresetPanel::selectPreset(int presetIndex, bool edit)
{
    if (isValidIndex(presetIndex))
    {
        mListCtrl->SetItemState(
            presetIndex, wxLIST_STATE_FOCUSED | wxLIST_STATE_SELECTED, wxLIST_STATE_FOCUSED | wxLIST_STATE_SELECTED);
...
void UIElementPresetPanel::onImportCommand(wxCommandEvent& event)
{
    ...
    wxString path = wxFileSelector(_T("Choose preset file to import"), lastPath.c_str(), wxEmptyString, wxEmptyString,
        wxString::Format(_T("Preset files (*.spe)|*.spe|All files (%s)|%s"), wxFileSelectorDefaultWildcardStr,
...
void UIElementPresetPanel::onExportCommand(wxCommandEvent& event)
{
    ...
    wxString path = wxFileSelector(_T("Choose filename to export preset"), lastPath.c_str(), wxEmptyString,
        wxEmptyString, wxString::Format(_T("Preset files (*.spe)|*.spe|All files (%s)|%s"),
