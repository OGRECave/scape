...
// ----------------------------------------------------------------------------
UIElementPresetPanel::UIElementPresetPanel(wxWindow* parentWindow, const wxString& name, const wxString& caption, UIElementPresetPanelListener* listener, wxAuiManager* auiManager)
...
void UIElementPresetPanel::onItemSelected(wxListEvent &event)
{
	...
	bool enable = true;//(mListCtrl->GetItemText(mSelectedPresetIndex).compare(wxString(Utils::defaultNameString)) != 0);
...
void UIElementPresetPanel::onImportCommand(wxCommandEvent& event)
{
    ...
    wxString path = wxFileSelector(
		_T("Choose preset file to import"),
		lastPath.c_str(), 
		wxEmptyString,
		wxEmptyString,
		wxString::Format
		(
			_T("Preset files (*.spe)|*.spe|All files (%s)|%s"),
...
void UIElementPresetPanel::onExportCommand(wxCommandEvent& event)
{
    ...
    wxString path = wxFileSelector(
		_T("Choose filename to export preset"),
		lastPath.c_str(), 
		wxEmptyString,
		wxEmptyString,
		wxString::Format
