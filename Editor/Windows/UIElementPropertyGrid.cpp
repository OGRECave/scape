...
// ----------------------------------------------------------------------------
UIElementPropertyGrid::UIElementPropertyGrid(wxWindow* parent, const wxString& name, const wxString& caption,
    UIElementPropertyGridListener* listener, wxAuiManager* auiManager)
    : wxPanel(parent, wxID_ANY, wxDefaultPosition, wxDefaultSize, 0, wxPanelNameStr)
    , mName(name)
    , mGridManager(NULL)
    , mListener(listener)
...
// ----------------------------------------------------------------------------
void UIElementPropertyGrid::onSize(wxSizeEvent& event)
{
    mGridManager->SetSize(event.GetSize());
}
...
void UIElementPropertyGrid::onPropertyGridChange(wxPropertyGridEvent& event)
{
    ...
    if (mListener)
    {
        ...
        mGridManager->SetPropertyValue(id, mListener->onUpdatePropertyGridPropertyValue(mName, name, value));
    }
}
...
void UIElementPropertyGrid::setValues(const StringStringMap& valueMap)
{
    ...
    {
        wxString value(
            mListener->onUpdatePropertyGridPropertyValue(mName, propertyIt->first, wxString(propertyIt->second)));
