...
// ----------------------------------------------------------------------------
UIElementToolBar::UIElementToolBar(const wxString& toolBarName, wxWindow* parentWindow,
                                   UIElementToolBarListener* listener, wxAuiManager* auiManager)
    : wxToolBar(parentWindow, wxID_ANY, wxDefaultPosition, wxDefaultSize, wxTB_FLAT |
                                                                              ...
                                                                              // wxTB_RIGHT |
                                                                              0),
      mToolBarName(toolBarName), mAuiManager(auiManager), mListener(listener)
{
...
void UIElementToolBar::populate(const UIElementToolBarItemList& itemList)
{
    ...
    for (itemIt = itemList.begin(); itemIt != itemItEnd; ++itemIt)
    {
        ...
        AddRadioTool(toolId, item.label, Utils::getBitmap(item.icon), wxNullBitmap, item.label,
                     item.description);
