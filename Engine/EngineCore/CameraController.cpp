...
CameraController::CameraController() :
	mRenderView(NULL),
	mStartGrabRayIntersectionPosition(0.0, 0.0, 0.0),
	mCurrent2DGrabPosition(0.0, 0.0),
	mCameraTransform(),
	mCameraEulerAngles(0.0, 0.0, 0.0),
...
void CameraController::attach(RenderView *renderView)
{
	...
	mCameraEulerAngles = DVector3(cameraOrientation.getPitch(true).valueRadians(), cameraOrientation.getYaw().valueRadians(), cameraOrientation.getRoll().valueRadians());
...
void CameraController::onFrameTick()
{
	...
	bool grabPanTiltButtonPressed = inputManager->getButton(ButtonId::CAMERA_POINTERGRABPANTILT)->isPressed();
	bool grabDollyButtonPressed = inputManager->getButton(ButtonId::CAMERA_POINTERGRABDOLLY)->isPressed();

	bool cameraInputActive;
	cameraInputActive = camPanTiltButtonPressed || grabTruckButtonPressed || grabPanTiltButtonPressed || grabDollyButtonPressed;
	cameraInputActive = cameraInputActive || inputManager->getButton(ButtonId::CAMERA_KEYLEFT)->isPressed();
	cameraInputActive = cameraInputActive || inputManager->getButton(ButtonId::CAMERA_KEYRIGHT)->isPressed();
	...
	cameraInputActive = cameraInputActive || inputManager->getButton(ButtonId::CAMERA_KEYFORWARD)->isPressed();
	cameraInputActive = cameraInputActive || inputManager->getButton(ButtonId::CAMERA_KEYBACKWARD)->isPressed();
	cameraInputActive = cameraInputActive || inputManager->getAnalogInput(AnalogInputId::POINTER_MOVEZ) != 0;
	...
	std::pair<RenderView*, Ogre::Vector2> focusedRenderView2DPoint = inputManager->getInputPointer()->getCurrentState().getRenderView2DPoint();
	if (focusedRenderView2DPoint.first == mRenderView)
	{
		...
		if (cameraInputActive)
		{
			...
			{
				mCurrent2DGrabPosition = DVector2(focusedRenderView2DPoint.second.x, focusedRenderView2DPoint.second.y);
				mStartGrabRayIntersectionPosition = point;
			}

			if (camPanTiltButtonPressed || grabPanTiltButtonPressed) doPanTilt();

			// Start with trucking when the 2d mouse grab is used
			if (pointerGrabActive) doGrabTruck(grabTruckButtonPressed && !grabDollyButtonPressed);
			...
			if (grabDollyButtonPressed) doGrabDolly();
			...
			if (grabPanTiltButtonPressed || grabDollyButtonPressed) doGrabPanTilt();

			// As the tilt angle is bound, correct any remaining grab error using camera translation
			if (justPointerGrabActive) doGrabTruck(false);
...
void CameraController::doGrabTruck(bool update2DGrabPoint)
{
	...
	if (update2DGrabPoint)
	{
		...
		IVector2 deltaUnits(inputManager->getAnalogInput(AnalogInputId::POINTER_DELTAX), inputManager->getAnalogInput(AnalogInputId::POINTER_DELTAY));
	...
	DVector3 pointerDirection = getViewportPickRayDirection(DVector3(mCurrent2DGrabPosition.x, mCurrent2DGrabPosition.y, 0));
	...
	/* //TODO GdCarpentier 7/16/2007 : remove
		...
		if (node == NULL)
		{
			node = getEngineCore()->getSceneManager()->getRootSceneNode()->createChildSceneNode(_T("node"));
...
void CameraController::doGrabPanTilt()
{
	...
	DVector3 correctionEulerAngles = Utils::getYawPitchFromVectorDifference(pointerDirection, grabDirection);
...
void CameraController::doGrabDolly()
{
	...
	mCameraTransform.col3.asVector3() = mStartGrabRayIntersectionPosition + (mCameraTransform.col3.asVector3() - mStartGrabRayIntersectionPosition) * ratio;
}
...
void CameraController::updateTruck()
{
	...
	{
		mCameraTransform.col3 = mCameraTransform.col3 - moveSpeed * mCameraTransform.col0 * getTimeSinceLastFrame;
	}
	...
	{
		mCameraTransform.col3 = mCameraTransform.col3 + moveSpeed * mCameraTransform.col1 * getTimeSinceLastFrame;
	}
...
DVector3 CameraController::normalizeEulerAngles(const DVector3& eulerAngles)
{
	...
	while (result.y > Ogre::Math::PI) result.y -= Ogre::Math::TWO_PI;
	while (result.y < -Ogre::Math::PI) result.y += Ogre::Math::TWO_PI;
