...
void EngineCore::loadApplicationSettings()
{
    ...
    Ogre::LogManager::getSingleton().logMessage(
        _T("Loaded application settings from '") + string(GLOBALSETTINGS_FILENAME) + _T("'"));
...
void EngineCore::resetHeightfield()
{
    ...
    for (int pageRowIndex = 0; pageRowIndex < heightfieldBuffer->getHeightfieldBufferSet()->getPageRowCount();
         ++pageRowIndex)
    {
        for (int pageColumnIndex = 0;
             pageColumnIndex < heightfieldBuffer->getHeightfieldBufferSet()->getPageColumnCount(); ++pageColumnIndex)
        {
            ...
            Ogre::Real* data
                = new Ogre::Real[heightfieldBuffer->getHeightfieldBufferSet()->getElementColumnCountPerPage()
                    * heightfieldBuffer->getHeightfieldBufferSet()->getElementRowCountPerPage()];
            Ogre::PixelBox pixelBox(heightfieldBuffer->getHeightfieldBufferSet()->getElementColumnCountPerPage(),
                heightfieldBuffer->getHeightfieldBufferSet()->getElementRowCountPerPage(), 1, Ogre::PF_FLOAT32_R, data);
            ...
            for (int y = 0; y < heightfieldBuffer->getHeightfieldBufferSet()->getElementRowCountPerPage(); ++y)
            {
                for (int x = 0; x < heightfieldBuffer->getHeightfieldBufferSet()->getElementColumnCountPerPage(); ++x)
                {
                    ...
                    */
                    //						data[heightfieldBuffer->getElementRowCountPerPage()*y+x] = 255.0f * (Ogre::Real)((x +
                    //y) & 255);
                    float height = Utils::clamp(
                        data[heightfieldBuffer->getHeightfieldBufferSet()->getElementRowCountPerPage() * y + x], 0.0f,
                        255.0f);
                    height = 2 * height * height;
                    // height = height * 2.0f - 255.0f;
                    // height = height * height;
                    // height = 0.5f + 0.5f * height;
                    data[heightfieldBuffer->getHeightfieldBufferSet()->getElementRowCountPerPage() * y + x]
                        = 0.5f; // height;
                }
            }
    ...
    while (ti.hasMoreElements())
    {
        ...
        for (int side = 0; side < 6; ++side)
        {
            ...
            Utils::getTextureFromExternalFile(
                newTextureName, Ogre::ResourceGroupManager::DEFAULT_RESOURCE_GROUP_NAME, newTextureNames[side]);
