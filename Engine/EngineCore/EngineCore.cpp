...
EngineCore::EngineCore(EngineInterface* engineInterface) : 
		...
		mRoot(NULL),
		mDebugRenderWindow(NULL),
		mApplicationSettingsConfigFile(NULL),
		mSceneManager(NULL),
		mSettingsDatasetManager(NULL),
		mTickableManager(NULL),
		mRenderViewManager(NULL),
		mHeightfieldGeomTileVertexUVBufferManager(NULL),
		mHeightfieldGeomTileIndexBufferManager(NULL),
		mHeightfieldBufferSetManager(NULL),
		mInputManager(NULL),
		mSceneLoaded(false),
		mSceneManagerLoaded(false),
		mFrameTimerMilliseconds(0),
		mLastFrameTimerMilliseconds(0),
		mTimeSinceLastFrame(0.0f),
		mHeightfieldGeomManager(NULL),
		mHeightfieldOperationFactory(NULL),
		mHeightfieldOperationStack(NULL),
		mHeightfieldBrushManager(NULL),
		mHeightfieldFileEncoderManager(NULL),
		mHeightfieldFileDecoderManager(NULL),
		mSkySettings(NULL),
...
void EngineCore::loadApplicationSettings()
{
	...
	Ogre::LogManager::getSingleton().logMessage(_T("Loaded application settings from '") + string(GLOBALSETTINGS_FILENAME) + _T("'"));
...
void EngineCore::loadResourceLocations()
{
	...
	Ogre::LogManager::getSingleton().logMessage("Loaded resource paths from '" + string(RESOURCES_FILENAME) + "'");
...
void EngineCore::resetHeightfield()
{
	...
	for (int pageRowIndex = 0; pageRowIndex < heightfieldBuffer->getHeightfieldBufferSet()->getPageRowCount(); ++pageRowIndex)
	{
		for (int pageColumnIndex = 0; pageColumnIndex < heightfieldBuffer->getHeightfieldBufferSet()->getPageColumnCount(); ++pageColumnIndex)
		{
			HeightfieldBufferPage* page = heightfieldBuffer->getPage(pageColumnIndex, pageRowIndex, HeightfieldBuffer::PAGEACCESSMODE_WRITE_DISCARD);
			Ogre::Real* data = new Ogre::Real[heightfieldBuffer->getHeightfieldBufferSet()->getElementColumnCountPerPage() * heightfieldBuffer->getHeightfieldBufferSet()->getElementRowCountPerPage()];
			Ogre::PixelBox pixelBox(heightfieldBuffer->getHeightfieldBufferSet()->getElementColumnCountPerPage(), heightfieldBuffer->getHeightfieldBufferSet()->getElementRowCountPerPage(), 1, Ogre::PF_FLOAT32_R, data);
			...
			for (int y = 0; y < heightfieldBuffer->getHeightfieldBufferSet()->getElementRowCountPerPage(); ++y)
			{
				for (int x = 0; x < heightfieldBuffer->getHeightfieldBufferSet()->getElementColumnCountPerPage(); ++x)
				{
					data[heightfieldBuffer->getHeightfieldBufferSet()->getElementColumnCountPerPage()*y+x] = 0.0f;
/*
					float power = 0.05f;
					for (int level = 11; level >= 0; level--)
					{
						...
						int rpx2 = Utils::randHash(px + spacing);
						int rpy2 = Utils::randHash(py + spacing * 13);
						int h1 = Utils::randHash(rpx + rpy);
						int h2 = Utils::randHash(rpx2 + rpy);
						int h3 = Utils::randHash(rpx + rpy2);
						int h4 = Utils::randHash(rpx2 + rpy2);
						int wx = ((x + relX) & invmask) * 4096 / spacing;
						int wy = ((y + relY) & invmask) * 4096 / spacing;
						int hy1 = ((h1 << 12) + (h2 - h1) * wx) >> 12;
						int hy2 = ((h3 << 12) + (h4 - h3) * wx) >> 12;
						int h = ((hy1 << 12) + (hy2 - hy1) * wy) >> 12;

						float height = power * (h / 4096.0f - 0.5f);
						power = power * 0.45f + 0.0000025f;
						data[heightfieldBuffer->getElementRowCountPerPage()*y+x] += height;
					}
*/
//						data[heightfieldBuffer->getElementRowCountPerPage()*y+x] = 255.0f * (Ogre::Real)((x + y) & 255);
					float height = Utils::clamp(data[heightfieldBuffer->getHeightfieldBufferSet()->getElementRowCountPerPage()*y+x], 0.0f, 255.0f);
					height = 2 * height*height;
					//height = height * 2.0f - 255.0f;
					//height = height * height;
					//height = 0.5f + 0.5f * height;
					data[heightfieldBuffer->getHeightfieldBufferSet()->getElementRowCountPerPage()*y+x] = 0.5f;//height;
				}
			}
...
// ----------------------------------------------------------------------------
Ogre::SceneManager* EngineCore::getSceneManager()
{
...
// ----------------------------------------------------------------------------
float EngineCore::getTimeSinceLastFrame() const
{
...
void EngineCore::update()
{
	if (getSceneManager())
	{
		...
		mTimeSinceLastFrame = Utils::max(0.0005f, 0.001f * (newFrameTickMilliseconds - mFrameTimerMilliseconds)); 
		...
		*/
...

		{
			...
			if (!createdGeometry)
			{
				...
				HeightfieldBufferSet* heigtfieldBufferSet = getHeightfieldBufferSetManager()->get(getHeightfieldBufferSetManager()->findHandle(_T("Project")));
				getHeightfieldGeomManager()->create(heigtfieldBufferSet->get(heigtfieldBufferSet->findHandle(_T("Render"))), node);
			
				/*
				Ogre::SceneNode* node2 = getEngineCore()->getSceneManager()->getRootSceneNode()->createChildSceneNode(_T("spherenode"));
				...
				Ogre::SceneNode* node3 = getEngineCore()->getSceneManager()->getRootSceneNode()->createChildSceneNode(_T("spherenode2"));
		...
		/*
		{
			...
			{
				Ogre::Viewport* viewport = getInputManager()->getInputPointer()->getCurrentState().getRenderView2DPoint().first->getViewport();
				//static bool boundCompositor = false;
				if (viewport)
				{
					Ogre::CompositorInstance* compInst = Ogre::CompositorManager::getSingleton().addCompositor(viewport, _T("B&W"));
					Ogre::CompositorManager::getSingleton().setCompositorEnabled(viewport, _T("B&W"), true);
...
// ----------------------------------------------------------------------------
EngineCore& EngineCore::getSingleton()
{
	return *msSingleton;
}
...
Ogre::RenderWindow* EngineCore::getDebugRenderWindow()
{
	...
	{
		mDebugRenderWindow = Ogre::Root::getSingleton().createRenderWindow("GPU2DOperationTarget", 512, 512, false);
	}
...
// ----------------------------------------------------------------------------
void EngineCore::onSkySettingsUpdate()
{
	loadSkyBox();
}
