...
// ----------------------------------------------------------------------------
void EngineInterface::createRenderView(int viewId, const std::string &windowHandle, int left, int top, int width, int height)
{
	getEngineCore()->getRenderViewManager()->createRenderView(viewId, windowHandle, left, top, width, height);
}
...
// ----------------------------------------------------------------------------
void EngineInterface::openConsole()
{
	Console::openConsole();
}
...
bool EngineInterface::selectOperation(const string& operationName)
{
	...
	{
		Ogre::LogManager::getSingleton().logMessage(_T("EngineInterface::selectOperation(): unknown specified operation '") + operationName + _T("'"));
		return false;
	}
}
...
// ----------------------------------------------------------------------------
string EngineInterface::getUIElementPropertyField(const string& elementName, const string& propertyName, const string& propertyFieldName)
{
	return getEngineCore()->getSettingsDatasetManager()->getSetting(_T("UI"), elementName, propertyName, propertyFieldName);
}
...
void EngineInterface::writeAuxiliaryFile(const string& fileName, const void* data, int bytes)
{
	...
	if (getEngineCore()) path = getEngineCore()->getApplicationSetting("Paths", "AuxiliarySettings");
	if (path.length()) path = Ogre::StringUtil::standardisePath(path);
...
// ----------------------------------------------------------------------------
StringStringMap EngineInterface::getUIElementPropertyValueMap(EScapeUIElementGroupId groupId, const string& elementName)
{
...
// ----------------------------------------------------------------------------
string EngineInterface::getUIElementPresetPropertyValue(EScapeUIElementGroupId groupId, const string& elementName, const string& presetName, const string& propertyName)
{
	UIElementPresetContainer* container = dynamic_cast<UIElementPresetContainer*>(getUIElementContainer(groupId, elementName));
...
// ----------------------------------------------------------------------------
void EngineInterface::setUIElementPresetPropertyValue(EScapeUIElementGroupId groupId, const string& elementName, const string& presetName, const string& propertyName, const string& value)
{
	UIElementPresetContainer* container = dynamic_cast<UIElementPresetContainer*>(getUIElementContainer(groupId, elementName));
...
// ----------------------------------------------------------------------------
StringStringMap EngineInterface::getUIElementPresetPropertyValueMap(EScapeUIElementGroupId groupId, const string& elementName, const string& presetName)
{
	UIElementPresetContainer* container = dynamic_cast<UIElementPresetContainer*>(getUIElementContainer(groupId, elementName));
	if (container)
	{
...
// ----------------------------------------------------------------------------
void EngineInterface::setUIElementPresetPropertyValueMap(EScapeUIElementGroupId groupId, const string& elementName, const string& presetName, const StringStringMap& valueMap)
{
	UIElementPresetContainer* container = dynamic_cast<UIElementPresetContainer*>(getUIElementContainer(groupId, elementName));
	if (container)
	{
...
// ----------------------------------------------------------------------------
StringStringMap EngineInterface::setUIElementPropertyValueMap(EScapeUIElementGroupId groupId, const string& elementName, const StringStringMap& valueMap)
{
	UIElementContainer* container = getUIElementContainer(groupId, elementName);
...
// ----------------------------------------------------------------------------
StringList EngineInterface::getUIElementPresetPropertyNames(EScapeUIElementGroupId groupId, const string& elementName)
{
	UIElementPresetContainer* container = dynamic_cast<UIElementPresetContainer*>(getUIElementContainer(groupId, elementName));
...
// ----------------------------------------------------------------------------
void EngineInterface::deleteUIElementPreset(EScapeUIElementGroupId groupId, const string& elementName, const string& presetName)
{
	UIElementPresetContainer* container = dynamic_cast<UIElementPresetContainer*>(getUIElementContainer(groupId, elementName));
	if (container)
	{
...
// ----------------------------------------------------------------------------
string EngineInterface::importUIElementPreset(EScapeUIElementGroupId groupId, const string& elementName, const string& fileName)
{
	UIElementPresetContainer* container = dynamic_cast<UIElementPresetContainer*>(getUIElementContainer(groupId, elementName));
	if (container)
	{
...
// ----------------------------------------------------------------------------
void EngineInterface::exportUIElementPreset(EScapeUIElementGroupId groupId, const string& elementName, const string& fileName)
{
	UIElementPresetContainer* container = dynamic_cast<UIElementPresetContainer*>(getUIElementContainer(groupId, elementName));
	if (container)
	{
...
// ----------------------------------------------------------------------------
string EngineInterface::makeUniquePresetName(EScapeUIElementGroupId groupId, const string& elementName, const string& baseName)
{
	UIElementPresetContainer* container = dynamic_cast<UIElementPresetContainer*>(getUIElementContainer(groupId, elementName));
	if (container)
	{
...
bool EngineInterface::exportImageFile(const string& encoderName, const string& fileName, string* error)
{
	...
	if (geom)
	{
		...
		HeightfieldFileEncoder* encoder = getEngineCore()->getHeightfieldFileEncoderManager()->getEncoder(encoderName);
		if (encoder)
		{
...
bool EngineInterface::importImageFile(const string& decoderName, const string& fileName, string* error)
{
	...
	if (geom)
	{
		...
		HeightfieldFileDecoder* decoder = getEngineCore()->getHeightfieldFileDecoderManager()->getDecoder(decoderName);
		if (decoder)
		{
...
// ----------------------------------------------------------------------------
StringList EngineInterface::getUIElementPropertyNameList(EScapeUIElementGroupId groupId, const string& elementName)
{
	StringList list;
...
// ----------------------------------------------------------------------------
string EngineInterface::getUIElementPropertyValue(EScapeUIElementGroupId groupId, const string& elementName, const string& propertyName)
{
	UIElementContainer* container = getUIElementContainer(groupId, elementName);
...
// ----------------------------------------------------------------------------
string EngineInterface::setUIElementPropertyValue(EScapeUIElementGroupId groupId, const string& elementName, const string& propertyName, const string& value)
{
	UIElementContainer* container = getUIElementContainer(groupId, elementName);
...
// ----------------------------------------------------------------------------
UIElementContainer* EngineInterface::getUIElementContainer(EScapeUIElementGroupId groupId, const string& elementName)
{
	...
	case SCAPEUIELEMENTGROUPID_RENDERWINDOW:
		{
			...
			{
				container = getEngineCore()->getRenderViewManager()->getRenderView(getEngineCore()->getRenderViewManager()->getCurrentRenderViewId());
			}
		} break;
