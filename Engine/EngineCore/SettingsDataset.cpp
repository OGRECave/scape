...
SettingsDataset::~SettingsDataset()
{
    ...
    for (sectionIt = mSectionMapStruct->map.begin(); sectionIt != sectionItEnd; ++sectionIt)
    {
        ...
        SubsectionMapStruct::MapType::iterator subsectionIt, subsectionItEnd = subsectionMapStruct->map.end();
        for (subsectionIt = subsectionMapStruct->map.begin(); subsectionIt != subsectionItEnd; ++subsectionIt)
        {
...
// ----------------------------------------------------------------------------
SettingsDataset::KeyIterator SettingsDataset::getKeyIterator(const string& section, const string& subsection)
{
    SectionMapStruct::MapType::const_iterator sectionIt = mSectionMapStruct->map.find(section);
    if (sectionIt != mSectionMapStruct->map.end())
    {
...
// ----------------------------------------------------------------------------
void SettingsDataset::setSetting(
    const string& section, const string& subsection, const string& key, const string& value)
{
    SectionMapStruct::MapType::iterator sectionIt = mSectionMapStruct->map.find(section);
    if (sectionIt == mSectionMapStruct->map.end())
    {
        sectionIt = mSectionMapStruct->map.insert(
            mSectionMapStruct->map.begin(), SectionMapStruct::MapType::value_type(section, new SubsectionMapStruct()));
    }
    SubsectionMapStruct* subsectionMapStruct = sectionIt->second;
    ...
    if (subsectionIt == subsectionMapStruct->map.end())
    {
        subsectionIt = subsectionMapStruct->map.insert(
            subsectionMapStruct->map.begin(), SubsectionMapStruct::MapType::value_type(subsection, new KeyMapStruct()));
    }
    KeyMapStruct* keyMapStruct = subsectionIt->second;
        ...
        if (lineLength > 0)
        {
            ...
            else if (correctDataset)
            {
                ...
                if (separator_pos != std::string::npos)
                {
                    ...
                    {
                        SectionMapStruct::MapType::iterator sectionIt = mSectionMapStruct->map.find(sectionName);
                        if (sectionIt == mSectionMapStruct->map.end())
                        {
                            sectionIt = mSectionMapStruct->map.insert(mSectionMapStruct->map.begin(),
                                SectionMapStruct::MapType::value_type(sectionName, new SubsectionMapStruct()));
                        }
                    ...
                    if (!keyMapStruct)
                    {
                        ...
                        if (subsectionIt == subsectionMapStruct->map.end())
                        {
                            subsectionIt = subsectionMapStruct->map.insert(subsectionMapStruct->map.begin(),
                                SubsectionMapStruct::MapType::value_type(subsectionName, new KeyMapStruct()));
                        }
    ...
    for (sectionIt = mSectionMapStruct->map.begin(); sectionIt != sectionItEnd; ++sectionIt)
    {
        ...
        for (subsectionIt = subsectionMapStruct->map.begin(); subsectionIt != subsectionItEnd; ++subsectionIt)
        {
            ...
            for (keyIt = keyMapStruct->map.begin(); keyIt != keyItEnd; ++keyIt)
            {
                ...
                assert(keyIt->first.length() && keyIt->first.find_first_of(assignmentInfix) == std::string::npos);
