...
GPU2DOperationManager::~GPU2DOperationManager()
{
    ...
    for (mapIt = mGPU2DOperationMap.begin(); mapIt != mapItEnd; ++mapIt)
    {
        ...
        for (listIt = instanceList.begin(); listIt != listItEnd;)
        {
            ...
            Ogre::LogManager::getSingleton().logMessage(_T("Destroying GPU2DOperation rtt '") +
                                                        textureName + _T("'"));
                                                           ...
                                                           Ogre::PixelFormat pixelFormat)
{
    ...
    if (!gpu2DRenderPtr)
    {
        ...
        Ogre::LogManager::getSingleton().logMessage(_T("Creating GPU2DOperation rtt '") + textureName +
                                                    _T("'"));
        ...
        Ogre::TexturePtr renderTexture = Ogre::TextureManager::getSingleton().createManual(
            ...
            renderGroup.width, renderGroup.height, 1, renderGroup.pixelFormat, Ogre::TU_RENDERTARGET);
        ...
        */
        ...
        Ogre::LogManager::getSingleton().logMessage(
            "Total number of GPU2DOperation rtt textures: " +
        ...
        for (listIt = instanceList.begin(); listIt != listItEnd;)
        {
            if (listIt->ptr.use_count() == 1)
            {
                    ...
                    currentMillis)
                {
                    ...
                    Ogre::LogManager::getSingleton().logMessage(
                        _T("Destroying GPU2DOperation rtt '") + textureName + _T("' (untouched: ") +
                        Ogre::StringConverter::toString(currentMillis - listIt->timestamp) + _T(" ms )"));
                    Ogre::TextureManager::getSingleton().remove(textureName);
                    listIt = instanceList.erase(listIt);
                    --mTotalGPU2DOperationCount;
                    Ogre::LogManager::getSingleton().logMessage(
