...
int GPU2DOperationRenderableQuad::getMaterialShaderConstantIndex(const string& constantName, bool makeDirty)
{
    ...
    if (mapIt == mShaderConstantNameToIndexMap.end())
    {
        ...
        mShaderConstantNameToIndexMap.insert(
            ShaderConstantNameToIndexMap::value_type(constantName, std::pair<int, bool>(constantIndex, makeDirty)));
    }
    else
    {
...
// ----------------------------------------------------------------------------
void GPU2DOperationRenderableQuad::setMaterialShaderConstant(
    const string& constantName, const Ogre::Real* data, int dataCount)
{
...
//-----------------------------------------------------------------------
void GPU2DOperationRenderableQuad::setMaterialShaderConstantUVRect(
    const string& constantName, const Ogre::Rect& rect, bool flipX, bool flipY)
{
    Ogre::Vector4 uvTransform(1.0f / rect.width(), 1.0f / rect.height(), (Ogre::Real)(-rect.left) / rect.width(),
