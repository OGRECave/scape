...
// ----------------------------------------------------------------------------
const Ogre::Rect& GPU2DOperationRenderableQuad::getRenderRect()
{
	return mRenderRect;
}
...
int GPU2DOperationRenderableQuad::getMaterialShaderConstantIndex(const string& constantName, bool makeDirty)
{
	...
	if (mapIt == mShaderConstantNameToIndexMap.end())
	{
		...
		mShaderConstantNameToIndexMap.insert(ShaderConstantNameToIndexMap::value_type(constantName, std::pair<int, bool>(constantIndex, makeDirty)));
	}
	else
	{
...
// ----------------------------------------------------------------------------
void GPU2DOperationRenderableQuad::setMaterialShaderConstant(const string& constantName, const Ogre::Real* data, int dataCount)
{
...
// ----------------------------------------------------------------------------
void GPU2DOperationRenderableQuad::setMaterialShaderConstant(const string& constantName, const Ogre::Vector4& data)
{
	int constantIndex = getMaterialShaderConstantIndex(constantName, true);
...
// ----------------------------------------------------------------------------
void GPU2DOperationRenderableQuad::setMaterialShaderConstant(const string& constantName, const Ogre::Matrix4& data)
{
	int constantIndex = getMaterialShaderConstantIndex(constantName, true);
...
//-----------------------------------------------------------------------
void GPU2DOperationRenderableQuad::setMaterialShaderConstantUVRect(const string& constantName, const Ogre::Rect& rect, bool flipX, bool flipY)
{
	Ogre::Vector4 uvTransform(
		1.0f / rect.width(),
		1.0f / rect.height(),
		(Ogre::Real)(-rect.left) / rect.width(),
		(Ogre::Real)(-rect.top) / rect.height());

	if (flipX) {uvTransform.x = -uvTransform.x; uvTransform.z = 1.0f - uvTransform.z;}
	if (flipY) {uvTransform.y = -uvTransform.y; uvTransform.w = 1.0f - uvTransform.w;}
...
//-----------------------------------------------------------------------
void GPU2DOperationRenderableQuad::setMaterialTexture(const string& aliasName, const Ogre::TexturePtr& texturePtr)
{
...
void GPU2DOperationRenderableQuad::prepareForRender(Ogre::ushort zOrder)
{
	...
	ShaderConstantNameToIndexMap::iterator nameIndexPairIt, nameIndexPairItEnd = mShaderConstantNameToIndexMap.end();
	for (nameIndexPairIt = mShaderConstantNameToIndexMap.begin(); nameIndexPairIt != nameIndexPairItEnd; ++nameIndexPairIt)
	{
		if (nameIndexPairIt->second.second || newMaterial)
		{
			Utils::bindShaderCustomAutoConstant(technique, nameIndexPairIt->second.first, nameIndexPairIt->first);
	...
	if (mDirtyVertexBuffer)
	{
		...
		*pPos++ = (Ogre::Real)mRenderRect.left;	*pPos++ = (Ogre::Real)mRenderRect.top;
		*pPos++ = (Ogre::Real)mRenderRect.left;	*pPos++ = (Ogre::Real)mRenderRect.bottom;
		*pPos++ = (Ogre::Real)mRenderRect.right;*pPos++ = (Ogre::Real)mRenderRect.top;
		*pPos++ = (Ogre::Real)mRenderRect.right;*pPos++ = (Ogre::Real)mRenderRect.bottom;
