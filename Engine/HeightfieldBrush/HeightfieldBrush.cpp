...
// ----------------------------------------------------------------------------
void HeightfieldBrush::setRadius(Ogre::Real radius)
{
	mInnerRadius = mOuterRadius = radius;
}


// ----------------------------------------------------------------------------
void HeightfieldBrush::setPosition(const Ogre::Vector3& position)
{
	mPosition = position;
}
...
// ----------------------------------------------------------------------------
void HeightfieldBrush::setPressure(Ogre::Real pressure)
{
	mPressure = pressure;
}
...
// ----------------------------------------------------------------------------
void HeightfieldBrush::setRampPower(Ogre::Real rampPower)
{
...
void HeightfieldBrush::tick()
{
	...
	{
		Ogre::Real sizeChangeRatio = expf(-0.6f * getEngineCore()->getTimeSinceLastFrame() * (inputManager->getAnalogInput(AnalogInputId::POINTER_DELTAY) - inputManager->getAnalogInput(AnalogInputId::POINTER_DELTAX)));
	...
	if (inputManager->getButton(ButtonId::BRUSH_CHANGEFALLOFF)->isPressed())
	{
		...
		//sizeChangeRatio = pow(sizeChangeRatio, -inputManager->getAnalogInput(AnalogInputId::POINTER_DELTAY) + inputManager->getAnalogInput(AnalogInputId::POINTER_DELTAX));
		//Ogre::Real midRadius = 0.5f * (getOuterRadius() + getInnerRadius());
		//Ogre::Real delta = std::max(0.5f, std::min(0.5f * (getOuterRadius() - getInnerRadius()) * sizeChangeRatio, midRadius - 0.5f));
...
//	//mPrimaryActive = getHeightfieldGeom() && inputManager->getButton(ButtonId::BRUSH_ACTIVE)->isPressed();
	//mSecondaryActive = getHeightfieldGeom() && !mPrimaryActive && inputManager->getButton(ButtonId::BRUSH_ACTIVE_ALT)->isPressed();
