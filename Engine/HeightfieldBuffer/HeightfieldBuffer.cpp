...
// ----------------------------------------------------------------------------
HeightfieldBuffer::HeightfieldBuffer(HeightfieldBufferSet* set, const string& name, Ogre::PixelFormat pixelFormat,
    int elementColumnCountPerPage, int elementRowCountPerPage, int elementColumnCount, int elementRowCount,
...
void HeightfieldBuffer::cloneFrom(HeightfieldBuffer* heightfieldBuffer, bool sharePages)
{
    ...
    assert(heightfieldBuffer->mPages.size()
        == (size_t)(getHeightfieldBufferSet()->getPageRowCount() * getHeightfieldBufferSet()->getPageColumnCount()));
...
{
    if (pageColumnIndex >= 0 && pageColumnIndex < getHeightfieldBufferSet()->getPageColumnCount() && pageRowIndex >= 0
        && pageRowIndex < getHeightfieldBufferSet()->getPageRowCount())
    {
    ...
    while (needsAnotherPass)
    {
        ...
        for (int pageRowIndex = 0; pageRowIndex < getHeightfieldBufferSet()->getPageRowCount(); ++pageRowIndex)
        {
            for (int pageColumnIndex = 0; pageColumnIndex < getHeightfieldBufferSet()->getPageColumnCount();
                 ++pageColumnIndex)
            {
                needsAnotherPass = getPage(pageColumnIndex, pageRowIndex, PAGEACCESSMODE_READONLY)->revalidatePass()
                    || needsAnotherPass;
            }
        }
    }
    ...
    for (int pageRowIndex = 0; pageRowIndex < getHeightfieldBufferSet()->getPageRowCount(); ++pageRowIndex)
    {
        ...
        if (pageRect.height() > 0)
        {
                 ...
                 ++pageColumnIndex)
            {
                ...
                pageRect.right = std::min<long>(
                    pixelXStart + getHeightfieldBufferSet()->getElementColumnCountPerPage(), relativePixelBox.right);
