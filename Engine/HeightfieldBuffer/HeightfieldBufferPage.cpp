...
// ----------------------------------------------------------------------------
HeightfieldBufferPage::~HeightfieldBufferPage()
{
	clearData();
}
...
{
	std::list<HeightfieldBuffer*>::iterator bufferIt = mHeightfieldBuffers.begin(), bufferItEnd = mHeightfieldBuffers.end();
...
// ----------------------------------------------------------------------------
void HeightfieldBufferPage::removeAllHeightfieldBuffers()
{
	mHeightfieldBuffers.clear();
}
...
// ----------------------------------------------------------------------------
bool HeightfieldBufferPage::containsData()
{
	return !_mHeightTexture.isNull();
}
...
Ogre::TexturePtr HeightfieldBufferPage::getHeightTexture()
{
	if (!containsData())
	{
		...
		string textureName = _T("HeightfieldBufferPage") + Ogre::StringConverter::toString(createdPageNumber);
...
// ----------------------------------------------------------------------------
bool HeightfieldBufferPage::revalidatePass()
{
	return false;
}
...
void HeightfieldBufferPage::cloneDataFrom(HeightfieldBufferPage* const heightfieldBufferPage)
{
	...
	if (heightfieldBufferPage->containsData())
	{
		...
		//printf("Cloned pagedata '%s' to '%s'\n", heightfieldBufferPage->getHeightTexture()->getName().c_str(), getHeightTexture()->getName().c_str());
	}
	else
	{
		_mHeightTexture.setNull();
...
void HeightfieldBufferPage::updateFrom(const Ogre::PixelBox& relativePixelBox)
{
	...
	//Ogre::Rect invalidRect(relativePixelBox.left, relativePixelBox.top, relativePixelBox.right, relativePixelBox.bottom);

	increaseVersion();

	Ogre::Rect absoluteRect = getAbsoluteRect();

	updateGeoms(Utils::translatedRect(Utils::boxToRect(relativePixelBox), absoluteRect.left, absoluteRect.top));
...
Ogre::PixelFormat HeightfieldBufferPage::getPackedFormat()
{
	...
	{
	case Ogre::PF_SHORT_L: return Ogre::PF_R5G6B5;
	case Ogre::PF_SHORT_GR: return Ogre::PF_R8G8B8;
	case Ogre::PF_FLOAT32_R: return Ogre::PF_FLOAT32_R;
	case Ogre::PF_FLOAT32_GR: return Ogre::PF_FLOAT32_GR;
	case Ogre::PF_FLOAT32_RGBA: return Ogre::PF_FLOAT32_RGBA;
	default: assert(false && _T("HeightfieldBufferPage::getPackedFormat(): Unsupported format"));
	}
