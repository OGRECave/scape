...
HeightfieldBufferSet::HeightfieldBufferSet(const string& name, int elementColumnCount, int elementRowCount,
    ...
    : mName(name), mElementColumnCount(elementColumnCount), mElementRowCount(elementRowCount),
      mHeightRangeMin(heightRangeMin), mHeightRangeMax(heightRangeMax),
      mElementColumnCountPerPage(elementColumnCountPerPage), mElementRowCountPerPage(elementRowCountPerPage)
{
...
HeightfieldBufferHandle HeightfieldBufferSet::create(const string& name, Ogre::PixelFormat format)
{
    HeightfieldBuffer* heightfieldBuffer =
        new HeightfieldBuffer(this, name, format, mElementColumnCountPerPage, mElementRowCountPerPage,
                              mElementColumnCount, mElementRowCount, mHeightRangeMin, mHeightRangeMax);

    HeightfieldBufferHandle handle = heightfieldBuffer->getHandle();
    mHeightfieldBufferMap[handle] = heightfieldBuffer;

    Ogre::LogManager::getSingleton().logMessage(_T("Creating HeightfieldBuffer '") + name +
                                                _T("' in set '") + getName() + _T("'"));
                                                         ...
                                                         bool sharePages)
{
    ...
    if (originalBuffer)
    {
        ...
        {
            Ogre::LogManager::getSingleton().logMessage(_T("Creating HeightfieldBuffer '") + name +
                                                        _T("' from '") + originalBuffer->getName() +
                                                        _T("' in set '") + getName() + _T("'"));
...
bool HeightfieldBufferSet::destroy(HeightfieldBufferHandle handle)
{
    ...
    {
        Ogre::LogManager::getSingleton().logMessage(_T("Destroying HeightfieldBuffer '") +
                                                    it->second->getName() + _T("' in set '") + getName() +
                                                    _T("'"));
