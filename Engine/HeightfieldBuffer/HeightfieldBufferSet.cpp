...
HeightfieldBufferSet::HeightfieldBufferSet(const string& name, int elementColumnCount, int elementRowCount,
    Ogre::Real heightRangeMin, Ogre::Real heightRangeMax, int elementColumnCountPerPage, int elementRowCountPerPage)
    : mName(name)
...
{
    HeightfieldBuffer* heightfieldBuffer = new HeightfieldBuffer(this, name, format, mElementColumnCountPerPage,
        mElementRowCountPerPage, mElementColumnCount, mElementRowCount, mHeightRangeMin, mHeightRangeMax);

    HeightfieldBufferHandle handle = heightfieldBuffer->getHandle();
    mHeightfieldBufferMap[handle] = heightfieldBuffer;

    Ogre::LogManager::getSingleton().logMessage(
        _T("Creating HeightfieldBuffer '") + name + _T("' in set '") + getName() + _T("'"));

    return handle;
}

// ----------------------------------------------------------------------------
HeightfieldBufferHandle HeightfieldBufferSet::createFrom(
    const string& name, HeightfieldBufferHandle originalBufferHandle, bool sharePages)
{
    ...
    if (originalBuffer)
    {
        ...
        if (newBuffer)
        {
            Ogre::LogManager::getSingleton().logMessage(_T("Creating HeightfieldBuffer '") + name + _T("' from '")
                + originalBuffer->getName() + _T("' in set '") + getName() + _T("'"));
...
bool HeightfieldBufferSet::destroy(HeightfieldBufferHandle handle)
{
    ...
    {
        Ogre::LogManager::getSingleton().logMessage(
            _T("Destroying HeightfieldBuffer '") + it->second->getName() + _T("' in set '") + getName() + _T("'"));
