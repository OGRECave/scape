...
// ----------------------------------------------------------------------------
bool HeightfieldFileDecoderImageBase::decode(HeightfieldBuffer* outBuffer, const string& fileName, string* error)
{
    ...
    if (fileData.first)
    {
        ...
        if (fileData.second >= sizeof(Utils::BitmapFileHeader) + sizeof(Utils::BitmapInfoHeader))
        {
            ...
            {
                rawPixelBox = Ogre::PixelBox(bmpIH.width, bmpIH.height, 1, Ogre::PF_SHORT_L, dataPtr + headerSize);
            }
        }
