...
// ----------------------------------------------------------------------------
StringStringStringPairMap HeightfieldFileDecoderManager::getFileFilterMap()
{
...
void HeightfieldFileDecoderManager::registerDecoder(HeightfieldFileDecoder* decoder)
{
	...
	mFileFilterMap[decoder->getClassName()] = std::pair<string, string>(decoder->getFileExtension(), decoder->getFileFilter());

	string& combinedFileFilterString = mFileFilterMap[Utils::emptyString].second;
	Ogre::vector<string>::type combinedFileFilters = Ogre::StringUtil::split(combinedFileFilterString, FILEFILTERSEPERATOR);
	...
	{
		bool unique = true;
		Ogre::vector<string>::iterator combinedFileFilterIt, combinedFileFilterItEnd = combinedFileFilters.end();
		for (combinedFileFilterIt = combinedFileFilters.begin(); combinedFileFilterIt != combinedFileFilterItEnd; ++combinedFileFilterIt)
		{
		...
		if (unique)
		{
			if (combinedFileFilterString.length()) combinedFileFilterString += FILEFILTERSEPERATOR;
