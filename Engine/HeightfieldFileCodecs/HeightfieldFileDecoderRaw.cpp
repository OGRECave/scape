...
// ----------------------------------------------------------------------------
HeightfieldFileDecoderRaw::HeightfieldFileDecoderRaw() : mCodecExtension(_T("RAW")), mFlipX(false), mFlipY(false)
{
...
{
    static string fileExtension(_T("RAW"));
    return fileExtension;
}
const string& HeightfieldFileDecoderRaw::getFileFilter()
{
    static string fileFilter(_T("*.RAW;*.R8;*.R16;*.R32"));
    return fileFilter;
}
...
string HeightfieldFileDecoderRaw::setUIElementPropertyValue(const string& elementName, const string& value)
{
    ...
    else if (elementName.compare(UIELEMENTPROPERTY_BIGENDIAN) == 0)
    {
        mBigEndian = boolValue;
        outValue = Ogre::StringConverter::toString(boolValue);
    }
    else if (elementName.compare(UIELEMENTPROPERTY_BPP) == 0)
    {
        mBpp = intValue;
        outValue = Ogre::StringConverter::toString(intValue);
    }
    else if (elementName.compare(UIELEMENTPROPERTY_WIDTH) == 0)
    {
        mWidth = intValue;
        outValue = Ogre::StringConverter::toString(intValue);
    }
    else if (elementName.compare(UIELEMENTPROPERTY_HEIGHT) == 0)
    {
        mHeight = intValue;
        outValue = Ogre::StringConverter::toString(intValue);
    }
...
bool HeightfieldFileDecoderRaw::decode(HeightfieldBuffer* outBuffer, const string& fileName, string* error)
{
    Ogre::LogManager::getSingleton().logMessage(_T("Importing heightfield '") + outBuffer->getName()
        + _T("' from file '") + fileName + _T("' using decoder '") + getClassName() + _T("'"));
    assert(outBuffer);
    ...
    {
    case 0:
        pixelFormat = Ogre::PF_BYTE_A;
        break;
    case 1:
        pixelFormat = Ogre::PF_SHORT_L;
        break;
    case 2:
        pixelFormat = Ogre::PF_FLOAT32_R;
        break;
    default:
    {
        assert(false && _T("Unknown RAW format in HeightfieldFileDecoderRaw::decode()"));
        break;
    }
    }
...
// ----------------------------------------------------------------------------
bool HeightfieldFileDecoderRaw::decode(Ogre::PixelBox* pixelBox, bool flipX, bool flipY, bool bigEndian, string* error)
{
