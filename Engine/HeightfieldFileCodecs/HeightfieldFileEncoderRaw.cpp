...
// ----------------------------------------------------------------------------
bool HeightfieldFileEncoderRawBase::encode(HeightfieldBuffer* inBuffer, const string& fileName, string* error)
{
...
// ----------------------------------------------------------------------------
bool HeightfieldFileEncoderRawBase::encode(HeightfieldBuffer* inBuffer, FILE* fileHandle, bool flipX, bool flipY,
    bool bigEndian, Ogre::PixelFormat pixelFormat, string* error)
{
    if (fileHandle)
    {
        Ogre::PixelBox rawPixelBox(inBuffer->getHeightfieldBufferSet()->getElementColumnCount(),
            inBuffer->getHeightfieldBufferSet()->getElementRowCount(), 1, pixelFormat);
...
IMPLEMENT_HEIGHTFIELDFILEENCODERRAW_CLASS(
    HeightfieldFileEncoderRaw16L, _T("RAW"), _T("*.RAW;*.R16"), Ogre::PF_SHORT_L, false);
IMPLEMENT_HEIGHTFIELDFILEENCODERRAW_CLASS(
    HeightfieldFileEncoderRaw16B, _T("RAW"), _T("*.RAW;*.R16"), Ogre::PF_SHORT_L, true);
IMPLEMENT_HEIGHTFIELDFILEENCODERRAW_CLASS(
    HeightfieldFileEncoderRaw32F, _T("RAW"), _T("*.RAW;*.R32"), Ogre::PF_FLOAT32_R, false);
