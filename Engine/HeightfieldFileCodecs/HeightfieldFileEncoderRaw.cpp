...
// ----------------------------------------------------------------------------
string HeightfieldFileEncoderRawBase::setUIElementPropertyValue(const string& elementName, const string& value) 
{
	bool someValue = Ogre::StringConverter::parseBool(value);
	string outValue = Ogre::StringConverter::toString(someValue);

	if (elementName.compare(UIELEMENTPROPERTY_FLIPX) == 0) mFlipX = someValue;
	else if (elementName.compare(UIELEMENTPROPERTY_FLIPY) == 0) mFlipY = someValue;
...
// ----------------------------------------------------------------------------
bool HeightfieldFileEncoderRawBase::encode(
...
// ----------------------------------------------------------------------------
bool HeightfieldFileEncoderRawBase::encode(HeightfieldBuffer *inBuffer, FILE* fileHandle, bool flipX, bool flipY, bool bigEndian, Ogre::PixelFormat pixelFormat, string *error)
{
	if (fileHandle)
	{
		Ogre::PixelBox rawPixelBox(inBuffer->getHeightfieldBufferSet()->getElementColumnCount(), inBuffer->getHeightfieldBufferSet()->getElementRowCount(), 1, pixelFormat);
		...
		{
			Ogre::uint8* rowData = (Ogre::uint8*)rawPixelBox.data + (flipY ? lines - 1 - line : line) * rowBytePitch;
...
#define IMPLEMENT_HEIGHTFIELDFILEENCODERRAW_CLASS(className, extension, filter, format, bigendian) \
	className::className() : HeightfieldFileEncoderRawBase(format, bigendian) {LOADPROPERTIES_UIELEMENTCONTAINERSIMPLE_CLASS();} \
...
IMPLEMENT_HEIGHTFIELDFILEENCODERRAW_CLASS(HeightfieldFileEncoderRaw8, _T("RAW"), _T("*.RAW;*.R8"), Ogre::PF_L8, false);
IMPLEMENT_HEIGHTFIELDFILEENCODERRAW_CLASS(HeightfieldFileEncoderRaw16L, _T("RAW"), _T("*.RAW;*.R16"), Ogre::PF_SHORT_L, false);
IMPLEMENT_HEIGHTFIELDFILEENCODERRAW_CLASS(HeightfieldFileEncoderRaw16B, _T("RAW"), _T("*.RAW;*.R16"), Ogre::PF_SHORT_L, true);
IMPLEMENT_HEIGHTFIELDFILEENCODERRAW_CLASS(HeightfieldFileEncoderRaw32F, _T("RAW"), _T("*.RAW;*.R32"), Ogre::PF_FLOAT32_R , false);
