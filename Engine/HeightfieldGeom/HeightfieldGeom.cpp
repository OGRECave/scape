...
namespace ScapeEngine
{
	namespace HeightfieldGeomNS
	{
		...
		struct TileDistance
		{
			...
			inline bool operator==(const TileDistance &rhs) const
			{
				return	idealSpacing == rhs.idealSpacing;			
			}
...
:	mHeightfieldBuffer(heightfieldBuffer),
	mParentSceneNode(parentSceneNode),
	mVertexElementTypeUV(vertexElementTypeUV),
	mVertexElementTypeHeight(vertexElementTypeHeight),
	mGeomAutoConstantsDirty(true),
	mMillisLastRevalidate(0)
{
	...
	for (int y = 0; y < elementRows; y += tileSizeY)
	{
		...
		{
			//tileSizeX += 3;
			Ogre::Rect visibleRect = Utils::intersectTRect(Ogre::Rect(x, y, x + tileSizeX, y + tileSizeY), heightfieldQuadRect);
...
// ----------------------------------------------------------------------------
HeightfieldGeomTile *HeightfieldGeom::createTile(const Ogre::Rect& visibleQuadRect, size_t bufferedQuadColumnCount, size_t bufferedQuadRowCount)
{
	HeightfieldGeomTile* tile = new HeightfieldGeomTile(this, visibleQuadRect, bufferedQuadColumnCount, bufferedQuadRowCount);
	Ogre::AxisAlignedBox tileBox = tile->getBoundingBox();
	Ogre::Vector3 tileTopLeftPosition = mTopLeftPosition + Ogre::Vector3((Ogre::Real)visibleQuadRect.left, 0, (Ogre::Real)visibleQuadRect.top) * getWorldQuadSize();
...
void HeightfieldGeom::tick()
{
	...
	for (tileDistanceIt = tileDistanceSet.begin(); tileDistanceIt != tileDistanceItEnd && remainingBufferLODUpdates > 0; ++tileDistanceIt)
	{
		//Ogre::Rect visibleQuadRect = tileDistanceIt->tile->getVisibleQuadRect();
		//printf("Update LOD tile (%d,%d,%d,%d) : %d\n", visibleQuadRect.left, visibleQuadRect.top, visibleQuadRect.right, visibleQuadRect.bottom, tileDistanceIt->idealSpacing);
...
//	Ogre::Vector3 localRayDirection = (mParentSceneNode->_getDerivedOrientation().Inverse() * worldRay.getOrigin()).normalisedCopy();
//	Ogre::Vector3 localRayOrigin = worldRay.getOrigin() - mParentSceneNode->_getDerivedPosition();
	...
	if (abs(localRayDirection.x) < 1E-15) localRayDirection.x = 1E-15;
	if (abs(localRayDirection.z) < 1E-15) localRayDirection.z = 1E-15;

	const unsigned int elementColumnCount = getHeightfieldBuffer()->getHeightfieldBufferSet()->getElementColumnCountPerPage() * getHeightfieldBuffer()->getHeightfieldBufferSet()->getPageColumnCount();
	const unsigned int elementRowCount = getHeightfieldBuffer()->getHeightfieldBufferSet()->getElementRowCountPerPage() * getHeightfieldBuffer()->getHeightfieldBufferSet()->getPageRowCount();

	Ogre::AxisAlignedBox aabb;
	aabb.setMinimum(Ogre::Vector3(
		(Ogre::Real)0.0f,
		(Ogre::Real)getHeightfieldBuffer()->getHeightfieldBufferSet()->getHeightRangeMin(),
		(Ogre::Real)0.0f));

	aabb.setMaximum(Ogre::Vector3(
		(Ogre::Real)elementColumnCount,
	...
	if (Ogre::Math::intersects(localRay, aabb, &paramStart, &paramEnd) && paramEnd > paramStart)
	{
		...
		unsigned int currentColumn = Utils::clamp((int)floor(localRayOrigin.x), 0, (int)elementColumnCount - 2);
		...
		if (gridParamNextX <= 0) gridParamNextX += incrParamX;
		if (gridParamNextZ <= 0) gridParamNextZ += incrParamZ;
		...
		while (gridParam <= paramEnd && intersectionParam < 0.0f)
		{
			...
			if (gridParamNextX < gridParamNextZ)
			{
				...
				if (currentColumn >= elementColumnCount - 1) break;
			}
			else
			{
				...
				currentRow += incrRow;				
				if (currentRow >= elementRowCount - 1) break;
			}
		}
		//printf("\n");
	}
...
{
	const unsigned int elementColumnCountPerPage = getHeightfieldBuffer()->getHeightfieldBufferSet()->getElementColumnCountPerPage();
	const unsigned int elementRowCountPerPage = getHeightfieldBuffer()->getHeightfieldBufferSet()->getElementRowCountPerPage();

	size_t pageColumn = column / elementColumnCountPerPage;
	size_t pageRow = row / elementRowCountPerPage;
	HeightfieldBufferPage* page = getHeightfieldBuffer()->getPage(pageColumn, pageRow, HeightfieldBuffer::PAGEACCESSMODE_READONLY);
	...
	if (page)
	{
		...
		Ogre::Real scale = getHeightfieldBuffer()->getHeightfieldBufferSet()->getHeightRangeMax() - getHeightfieldBuffer()->getHeightfieldBufferSet()->getHeightRangeMin();
...
{
	// add extra border pixel to notify neighboring tiles when a normal at the dirtyrect's border potentially changed
