...
std::pair<bool, Ogre::Real> intersects(const Ogre::Ray& ray, const Ogre::Vector3& a, const Ogre::Vector3& b,
    const Ogre::Vector3& c, bool positiveSide = true, bool negativeSide = true)
{
    //
...
// ----------------------------------------------------------------------------
HeightfieldGeom::HeightfieldGeom(class HeightfieldBuffer* heightfieldBuffer, Ogre::SceneNode* parentSceneNode,
    Ogre::VertexElementType vertexElementTypeUV, Ogre::VertexElementType vertexElementTypeHeight)
...
// ----------------------------------------------------------------------------
HeightfieldGeomTile* HeightfieldGeom::createTile(
    const Ogre::Rect& visibleQuadRect, size_t bufferedQuadColumnCount, size_t bufferedQuadRowCount)
{
    HeightfieldGeomTile* tile
    ...
    Ogre::Vector3 tileTopLeftPosition = mTopLeftPosition
        + Ogre::Vector3((Ogre::Real)visibleQuadRect.left, 0, (Ogre::Real)visibleQuadRect.top) * getWorldQuadSize();
...
void HeightfieldGeom::tick()
{
    ...
    for (tileDistanceIt = tileDistanceSet.begin(); tileDistanceIt != tileDistanceItEnd && remainingBufferLODUpdates > 0;
         ++tileDistanceIt)
    {
...
Ogre::Real HeightfieldGeom::getIntersectionPoint(const Ogre::Ray& worldRay)
{
    ...
    const unsigned int elementRowCount = getHeightfieldBuffer()->getHeightfieldBufferSet()->getElementRowCountPerPage()
        * getHeightfieldBuffer()->getHeightfieldBufferSet()->getPageRowCount();

    Ogre::AxisAlignedBox aabb;
    aabb.setMinimum(Ogre::Vector3((Ogre::Real)0.0f,
        (Ogre::Real)getHeightfieldBuffer()->getHeightfieldBufferSet()->getHeightRangeMin(), (Ogre::Real)0.0f));

    aabb.setMaximum(Ogre::Vector3((Ogre::Real)elementColumnCount,
