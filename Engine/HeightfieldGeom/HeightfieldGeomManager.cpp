...
// ----------------------------------------------------------------------------
HeightfieldGeomManager::HeightfieldGeomManager() : mCurrentHeightfieldGeom(NULL)
{
    wantsPostFrameTick();
}

// ----------------------------------------------------------------------------
bool HeightfieldGeomManager::getClosestHeightfieldGeomRayIntersection(const Ogre::Ray& ray,
    const Ogre::SceneManager& sceneManager, class HeightfieldGeom*& outHeightfieldGeom, Ogre::Vector3& outPoint)
{
    HeightfieldGeoms::iterator heightfieldGeomIt = mHeightfieldGeoms.begin(),
                               heightfieldGeomItEnd = mHeightfieldGeoms.end();
...
HeightfieldGeom* HeightfieldGeomManager::create(HeightfieldBuffer* heightfieldBuffer, Ogre::SceneNode* rootNode)
{
    ...
    HeightfieldGeom* heightfieldGeom
        = new HeightfieldGeom(heightfieldBuffer, rootNode, uvVertexElementType, heightVertexElementType);
...
void HeightfieldGeomManager::onPostFrameTick()
{
    ...
    HeightfieldGeom* currentGeom
        = getEngineCore()->getInputManager()->getInputPointer()->getCurrentState().get3DPointGeom();
    if (currentGeom)
    {
