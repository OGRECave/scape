...
void HeightfieldGeomMaterialSplat::updateShaderConstantsGeom(
    const HeightfieldGeom* heightfieldGeom, ShaderCustomAutoConstants* shaderCustomAutoConstants, bool forceAll)
{
    ...
    if (forceAll)
    {
...

        {
            shaderCustomAutoConstants->setConstant(
                ...
                Ogre::Vector4((mIsoXZ ? mIsoFade : 0.0f), // amount of additive greyscale dep. on x iso output
                    ...
                    (mIsoXZ ? -mIsoFade : 0.0f)
                        + (mIsoY ? 0.5f * mIsoFade : 0.0f))); // amount of additive greyscale indep. of iso output
        }
...

        {
            shaderCustomAutoConstants->setConstant(
                ...
                Ogre::Vector4(1.0f / mUVScaleLocal, // scale from quad unit U to smaller hi-res texture U
                    ...
                    0.0f, // scale of distortion in U for hi-res texture. distortion comes from the red channel of the
                          // global texture
                    0.0f)); // scale of distortion in V for hi-res texture. distortion comes from the green channel of
                            // the global texture
        }

        {
            Ogre::Real layer2Lo = mLayerProperties[2].height
                - (mLayerProperties[2].heightUp ? 0.5f : -0.5f) * std::max(0.001f, mLayerProperties[2].heightVar);
            Ogre::Real layer2Hi = mLayerProperties[2].height
                + (mLayerProperties[2].heightUp ? 0.5f : -0.5f) * std::max(0.001f, mLayerProperties[2].heightVar);
            Ogre::Real layer3Lo = mLayerProperties[3].height
                - (mLayerProperties[3].heightUp ? 0.5f : -0.5f) * std::max(0.001f, mLayerProperties[3].heightVar);
            Ogre::Real layer3Hi = mLayerProperties[3].height
                + (mLayerProperties[3].heightUp ? 0.5f : -0.5f) * std::max(0.001f, mLayerProperties[3].heightVar);
            Ogre::Real layer4Lo = mLayerProperties[4].height
                - (mLayerProperties[4].heightUp ? 0.5f : -0.5f) * std::max(0.001f, mLayerProperties[4].heightVar);
            Ogre::Real layer4Hi = mLayerProperties[4].height
                + (mLayerProperties[4].heightUp ? 0.5f : -0.5f) * std::max(0.001f, mLayerProperties[4].heightVar);

            shaderCustomAutoConstants->setConstant(_HeightfieldGeomMaterialSplatNS::LAYERHEIGHTSCALE,
            ...
            shaderCustomAutoConstants->setConstant(_HeightfieldGeomMaterialSplatNS::LAYERHEIGHTBIAS,
        ...
        {
            Ogre::Real layer2SlopeLoTan = tan(0.01745f * (mLayerProperties[2].slope
                                                             - (mLayerProperties[2].slopeUp ? 0.5f : -0.5f)
                                                                 * std::max(0.001f, mLayerProperties[2].slopeVar)));
            Ogre::Real layer2SlopeHiTan = tan(0.01745f * (mLayerProperties[2].slope
                                                             + (mLayerProperties[2].slopeUp ? 0.5f : -0.5f)
                                                                 * std::max(0.001f, mLayerProperties[2].slopeVar)));
            Ogre::Real layer3SlopeLoTan = tan(0.01745f * (mLayerProperties[3].slope
                                                             - (mLayerProperties[3].slopeUp ? 0.5f : -0.5f)
                                                                 * std::max(0.001f, mLayerProperties[3].slopeVar)));
            Ogre::Real layer3SlopeHiTan = tan(0.01745f * (mLayerProperties[3].slope
                                                             + (mLayerProperties[3].slopeUp ? 0.5f : -0.5f)
                                                                 * std::max(0.001f, mLayerProperties[3].slopeVar)));
            Ogre::Real layer4SlopeLoTan = tan(0.01745f * (mLayerProperties[4].slope
                                                             - (mLayerProperties[4].slopeUp ? 0.5f : -0.5f)
                                                                 * std::max(0.001f, mLayerProperties[4].slopeVar)));
            Ogre::Real layer4SlopeHiTan = tan(0.01745f * (mLayerProperties[4].slope
                                                             + (mLayerProperties[4].slopeUp ? 0.5f : -0.5f)
                                                                 * std::max(0.001f, mLayerProperties[4].slopeVar)));
            ...
            shaderCustomAutoConstants->setConstant(
                ...
                Ogre::Vector4(-layer2SlopeLoTan / (layer2SlopeHiTan - layer2SlopeLoTan), // weight of layer2 (sand)
...

        {
            shaderCustomAutoConstants->setConstant(
                ...
                Ogre::Matrix4(10.0f * mLayerProperties[2].distortHi, 10.0f * mLayerProperties[2].distortMid,
                    10.0f * mLayerProperties[2].distortLo, 0.0f, 10.0f * mLayerProperties[3].distortHi,
                    10.0f * mLayerProperties[3].distortMid, 10.0f * mLayerProperties[3].distortLo, 0.0f,
                    10.0f * mLayerProperties[4].distortHi, 10.0f * mLayerProperties[4].distortMid,
                    10.0f * mLayerProperties[4].distortLo, 0.0f, 0.1f * mSlopeDistortHi, 0.1f * mSlopeDistortMid,
                    0.1f * mSlopeDistortLo, 0.0f));
        }

        {
            shaderCustomAutoConstants->setConstant(
                ...
                Ogre::Vector4(0.01f * mIntensityDistortHi, // intensity weight of red noise component
                    ...
                    1.0f
                        - 0.005f * (mIntensityDistortHi + mIntensityDistortMid
...

        {
            ...
            shaderCustomAutoConstants->setConstant(
                _HeightfieldGeomMaterialSplatNS::FOGCOLOR, Ogre::Vector4(fogColor.r, // fog color red
...

        {
            ...
            shaderCustomAutoConstants->setConstant(_HeightfieldGeomMaterialSplatNS::LIGHTDIRECTION,
                Ogre::Vector4(sunVec.x, // x component
                                                       sunVec.y, // y compnent
                                                       sunVec.z, // z component
                                                       0.1f * mLightAngleScale)); // multiplier for terrain gradient
                                                                                  // before being normalized and used in
...

        {
            ...
            shaderCustomAutoConstants->setConstant(
                _HeightfieldGeomMaterialSplatNS::SUNCOLOR, Ogre::Vector4(sunColor.r * sunOverbright, // sun color red
...

        {
            ...
            shaderCustomAutoConstants->setConstant(
                _HeightfieldGeomMaterialSplatNS::SKYCOLOR, Ogre::Vector4(skyColor.r * skyOverbright, // sky color red
...
void HeightfieldGeomMaterialSplat::updateShaderConstantsGeomTile(
    const HeightfieldGeomTile* heightfieldGeomTile, ShaderCustomAutoConstants* shaderCustomAutoConstants, bool forceAll)
{
...
void HeightfieldGeomMaterialSplat::updateTextures()
{
    ...
    Utils::getTextureFromExternalFile(
        TEXTUREBASENAME + mLayerProperties[1].textureFileName, group, mLayerProperties[1].textureFileName);
    Utils::getTextureFromExternalFile(
        TEXTUREBASENAME + mLayerProperties[2].textureFileName, group, mLayerProperties[2].textureFileName);
    Utils::getTextureFromExternalFile(
        TEXTUREBASENAME + mLayerProperties[3].textureFileName, group, mLayerProperties[3].textureFileName);
    Utils::getTextureFromExternalFile(
        TEXTUREBASENAME + mLayerProperties[4].textureFileName, group, mLayerProperties[4].textureFileName);
...
void HeightfieldGeomMaterialSplat::bindShaderConstants()
{
    ...
    Utils::bindShaderCustomAutoConstant(
        technique, _HeightfieldGeomMaterialSplatNS::LAYERHEIGHTSCALE, _T("layerHeightScale"));
    Utils::bindShaderCustomAutoConstant(
        technique, _HeightfieldGeomMaterialSplatNS::LAYERHEIGHTBIAS, _T("layerHeightBias"));
    Utils::bindShaderCustomAutoConstant(
        technique, _HeightfieldGeomMaterialSplatNS::LAYERHEIGHTDISTORT, _T("layerHeightDistort"));
    Utils::bindShaderCustomAutoConstant(
        technique, _HeightfieldGeomMaterialSplatNS::LAYERSLOPESCALE, _T("layerSlopeScale"));
    Utils::bindShaderCustomAutoConstant(
        technique, _HeightfieldGeomMaterialSplatNS::LAYERSLOPEBIAS, _T("layerSlopeBias"));
    Utils::bindShaderCustomAutoConstant(
        technique, _HeightfieldGeomMaterialSplatNS::INTENSITYSCALEGLOBAL, _T("intensityScaleGlobal"));
    Utils::bindShaderCustomAutoConstant(technique, _HeightfieldGeomMaterialSplatNS::FOGCOLOR, _T("fogColor"));
    Utils::bindShaderCustomAutoConstant(
        technique, _HeightfieldGeomMaterialSplatNS::LIGHTDIRECTION, _T("lightDirection"));
...
void HeightfieldGeomMaterialSplat::forceReloadTexture(const string& oldFileName, const string& newFileName)
{
    ...
    Utils::getTextureFromExternalFile(
        TEXTUREBASENAME + newFileName, Ogre::ResourceGroupManager::DEFAULT_RESOURCE_GROUP_NAME, newFileName);
}
