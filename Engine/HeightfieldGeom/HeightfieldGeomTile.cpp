...
HeightfieldGeomTile::HeightfieldGeomTile(HeightfieldGeom* heightfieldGeom,
    ...
    : Ogre::SimpleRenderable(), mDirtyData(false), mDirtyDataNeighbor(false), mDirtyMaterial(false),
      // mMaterial(),
      // mMaterialName(_T("HeightfieldMaterial")),
      mBoundingRadius(0), mHeightfieldGeom(heightfieldGeom), mVisibleQuadRect(visibleQuadRect),
      mTemplateQuadColumnCount(templateQuadColumnCount), mTemplateQuadRowCount(templateQuadRowCount),
    ...
    Ogre::GpuProgramParameters* params) const
{
    ...
    {
        if (!getHeightfieldGeom()->getGeomShaderCustomAutoConstants()->updateShaderParameter(params,
                                                                                             constantEntry))
        {
...
void HeightfieldGeomTile::revalidate()
{
    if (isDirtyData())
    {
        ...
        Ogre::Vector3 aabbHalfSize = 0.5f * Ogre::Vector3((Ogre::Real)mVisibleQuadRect.width(), heightRange,
                                                          (Ogre::Real)mVisibleQuadRect.height()) *
...
void HeightfieldGeomTile::updateRenderOperation()
{
    if (isDirtyData())
    {
        ...
        mRenderOp.vertexData->vertexBufferBinding->setBinding(vertexHeightElement.getSource(),
...
void HeightfieldGeomTile::_notifyCurrentCamera(Ogre::Camera* cam)
{
    ...
    float cameraOpening =
        atan(0.5f * cam->getFOVy().valueRadians()) / (0.5f * cam->getViewport()->getActualHeight());
    ...
    if (mFrameLeastSquaredPixelsWorldQuad < 0.0f ||
        squaredPixelsWorldQuad < mFrameLeastSquaredPixelsWorldQuad)
    {
...
int HeightfieldGeomTile::getBestLODSpacing() const
{
    if (mFrameLeastSquaredPixelsWorldQuad >= 0.0f)
    {
        Ogre::Real squaredPixelError =
            getHeightfieldGeom()->getHeightfieldGeomMaterial()->getLODPixelError(); // 10.0f;
        squaredPixelError *= squaredPixelError;

        Ogre::Real lodPower =
            getHeightfieldGeom()->getHeightfieldGeomMaterial()->getLODDistancePower(); // 1.0f;

        Ogre::Real lodChangeThreshold = 0.7f;

        Ogre::Real idealSpacing = std::max(
            1.0f, powf(1.0f + squaredPixelError * mFrameLeastSquaredPixelsWorldQuad, 0.005f * lodPower));
        ...
        {
            int spacing = std::max(
                1, (int)Ogre::Bitwise::firstPO2From((int)idealSpacing)); // rounded power-of-two number
