...
// ----------------------------------------------------------------------------
Ogre::HardwareVertexBufferSharedPtr HeightfieldGeomTileBufferFactory::createVertexHeightBuffer(
...
// ----------------------------------------------------------------------------
Ogre::HardwareVertexBufferSharedPtr HeightfieldGeomTileBufferFactory::createVertexUVBuffer(
    HeightfieldGeomTile* heightfieldGeomTile)
{
    HeightfieldGeom* heightfieldGeom = heightfieldGeomTile->getHeightfieldGeom();
    assert(heightfieldGeom);
    ...
    Ogre::HardwareVertexBufferSharedPtr vertexUVBuffer
        = getEngineCore()->getHeightfieldGeomTileVertexUVBufferManager()->getBuffer(
            ...
            heightfieldGeomTile->getTemplateQuadColumnCount(), heightfieldGeomTile->getTemplateQuadRowCount(),
            quadColumnSpacing, quadRowSpacing);

    return vertexUVBuffer;
}

// ----------------------------------------------------------------------------
Ogre::HardwareIndexBufferSharedPtr HeightfieldGeomTileBufferFactory::createIndexBuffer(
    HeightfieldGeomTile* heightfieldGeomTile)
{
    ...
    Ogre::HardwareIndexBufferSharedPtr indexBuffer
        = getEngineCore()->getHeightfieldGeomTileIndexBufferManager()->getBuffer(spacedVisibleQuadColumnCount,
            spacedVisibleQuadRowCount, std::max(1, spacingNorth / currentLodSpacing),
            std::max(1, spacingEast / currentLodSpacing), std::max(1, spacingSouth / currentLodSpacing),
...
Ogre::HardwareVertexBufferSharedPtr HeightfieldGeomTileBufferFactory::_createHeightBuffer(
    HeightfieldGeomTile* heightfieldGeomTile)
{
    ...
    Ogre::Rect availableDataRect(0, 0, heightfieldBuffer->getHeightfieldBufferSet()->getPageColumnCount()
    ...
    Ogre::Rect requestedPixelBoxRect(std::max<long>(0, visibleQuadRect.left), std::max<long>(0, visibleQuadRect.top),
    ...
    Ogre::Rect remainingPixelBoxRect = Ogre::Rect(requestedPixelBoxRect.left, requestedPixelBoxRect.top,
        requestedPixelBoxRect.right, requestedPixelBoxRect.bottom);
    ...
    while (remainingPixelBoxRect.top < remainingPixelBoxRect.bottom)
    {
        ...
        while (remainingPixelBoxRect.left < remainingPixelBoxRect.right)
        {
            HeightfieldBufferPage* page
                = heightfieldBuffer->getPage(heightfieldBuffer->getPageColumnFromElement(remainingPixelBoxRect.left),
    ...
    for (int row = visibleQuadRect.top; row < rowEnd; row += quadRowSpacing)
    {
        ...
        while (column < columnEnd)
        {
            ...
            const PageDataType* pixelRowData = (const PageDataType*)pixelBox.data
                + pageElementSize * (pixelBox.rowPitch * (row - pixelBox.top) - pixelBox.left);
            const PageDataType* pixelRowDXData = (const PageDataType*)pixelBoxDX.data
                + pageElementSize * (pixelBoxDX.rowPitch * (row - pixelBoxDX.top) - pixelBoxDX.left);
            const PageDataType* pixelRowDYData = (const PageDataType*)pixelBoxDY.data
            ...
            int columnInnerEnd
                = std::min<int>(visibleQuadRect.right, std::min<int>(pixelBox.right - 1, availableDataRect.right));
