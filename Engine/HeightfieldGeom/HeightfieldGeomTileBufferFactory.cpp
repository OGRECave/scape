...
// ----------------------------------------------------------------------------
Ogre::HardwareVertexBufferSharedPtr HeightfieldGeomTileBufferFactory::createVertexHeightBuffer(HeightfieldGeomTile* heightfieldGeomTile)
{
	HeightfieldGeom* heightfieldGeom = heightfieldGeomTile->getHeightfieldGeom();
	assert(heightfieldGeom);

	Ogre::VertexElementType elementBaseType = Ogre::VertexElement::getBaseType(heightfieldGeom->getVertexElementTypeHeight());
	Ogre::PixelFormat heightElementFormat = heightfieldGeom->getHeightfieldBuffer()->getHeightElementFormat();
...
// ----------------------------------------------------------------------------
Ogre::HardwareVertexBufferSharedPtr HeightfieldGeomTileBufferFactory::createVertexUVBuffer(HeightfieldGeomTile* heightfieldGeomTile)
{
	HeightfieldGeom* heightfieldGeom = heightfieldGeomTile->getHeightfieldGeom();
	assert(heightfieldGeom);
	...
	Ogre::HardwareVertexBufferSharedPtr vertexUVBuffer = 
		...
		heightfieldGeomTile->getTemplateQuadRowCount(),
		quadColumnSpacing,
		quadRowSpacing);
...
// ----------------------------------------------------------------------------
Ogre::HardwareIndexBufferSharedPtr HeightfieldGeomTileBufferFactory::createIndexBuffer(HeightfieldGeomTile* heightfieldGeomTile)
{
	...
	int spacedVisibleQuadColumnCount = (visibleQuadRect.width() + quadColumnSpacing - 1) / quadColumnSpacing;
	...
	std::list<HeightfieldGeomTile*>::const_iterator neighborTileIt, neighborTileItEnd = neighborTiles.end();
	for (neighborTileIt = neighborTiles.begin(); neighborTileIt != neighborTileItEnd; ++neighborTileIt)
	{
		...
		if (visibleQuadRect.top == neighborRect.bottom) spacingNorth = (*neighborTileIt)->getCurrentLODSpacing();
		else if (visibleQuadRect.right == neighborRect.left) spacingEast = (*neighborTileIt)->getCurrentLODSpacing();
		else if (visibleQuadRect.bottom == neighborRect.top) spacingSouth = (*neighborTileIt)->getCurrentLODSpacing();
		else if (visibleQuadRect.left == neighborRect.right) spacingWest = (*neighborTileIt)->getCurrentLODSpacing();
		else assert(false);
	}
	...
	Ogre::HardwareIndexBufferSharedPtr indexBuffer = 
		...
		spacedVisibleQuadColumnCount,
		spacedVisibleQuadRowCount,
		std::max(1, spacingNorth / currentLodSpacing), 
		std::max(1, spacingEast / currentLodSpacing), 
		std::max(1, spacingSouth / currentLodSpacing), 
		std::max(1, spacingWest / currentLodSpacing));
...
Ogre::HardwareVertexBufferSharedPtr HeightfieldGeomTileBufferFactory::_createHeightBuffer(HeightfieldGeomTile* heightfieldGeomTile)
{
	...
	Ogre::HardwareVertexBufferSharedPtr buffer = Ogre::HardwareBufferManager::getSingleton().createVertexBuffer(
		Ogre::VertexElement::getTypeSize(heightfieldGeom->getVertexElementTypeHeight()),
		vertexCount,
		Ogre::HardwareBuffer::HBU_STATIC_WRITE_ONLY, 
		false); 

	
	assert(sizeof(VertexDataType) == Ogre::VertexElement::getTypeSize(Ogre::VertexElement::getBaseType(heightfieldGeom->getVertexElementTypeHeight())));
	assert(sizeof(PageDataType) * Ogre::PixelUtil::getComponentCount(heightfieldBuffer->getHeightElementFormat()) == 
	...
	size_t vertexElementPtrPitch = Ogre::VertexElement::getTypeCount(heightfieldGeom->getVertexElementTypeHeight());

	Ogre::Rect availableDataRect(
		0,
		0,
		heightfieldBuffer->getHeightfieldBufferSet()->getPageColumnCount() * heightfieldBuffer->getHeightfieldBufferSet()->getElementColumnCountPerPage(),
		heightfieldBuffer->getHeightfieldBufferSet()->getPageRowCount() * heightfieldBuffer->getHeightfieldBufferSet()->getElementRowCountPerPage());

	typedef std::list<Ogre::PixelBox> PageElementRowBoxes;
	PageElementRowBoxes pageElementRowBoxes;

	Ogre::Rect requestedPixelBoxRect(
		std::max<long>(0, visibleQuadRect.left), 
	...
	Ogre::Rect remainingPixelBoxRect = Ogre::Rect(
			requestedPixelBoxRect.left, 
			requestedPixelBoxRect.top,
			requestedPixelBoxRect.right,
	...
	while (remainingPixelBoxRect.top < remainingPixelBoxRect.bottom)
	{
		...
		while (remainingPixelBoxRect.left < remainingPixelBoxRect.right)
		{
			...
			Ogre::Rect pixelBoxRect = Utils::intersectTRect(Utils::boxToRect(pixelBox), remainingPixelBoxRect);
	...
	for (int row = visibleQuadRect.top; row < rowEnd; row += quadRowSpacing)
	{
		...
		while (row >= (int)pagePixelBoxRowIt->begin()->bottom) pagePixelBoxRowIt++;
		while (rowDY >= (int)pagePixelBoxRowDYIt->begin()->bottom) pagePixelBoxRowDYIt++;
		...
		while (column < columnEnd)
		{
			...
			while (column >= (int)(*pixelBoxColumnIt).right) {pixelBoxColumnIt++; pixelBoxDYColumnIt++;}

			int columnDX = std::min<int>(column + 1, requestedPixelBoxRect.right - 1);
			while (columnDX >= (int)(*pixelBoxDXColumnIt).right) {pixelBoxDXColumnIt++;}
			...
			const PageDataType* pixelRowData = (const PageDataType*)pixelBox.data + pageElementSize * (pixelBox.rowPitch * (row - pixelBox.top) - pixelBox.left);
			const PageDataType* pixelRowDXData = (const PageDataType*)pixelBoxDX.data + pageElementSize * (pixelBoxDX.rowPitch * (row - pixelBoxDX.top) - pixelBoxDX.left);
			const PageDataType* pixelRowDYData = (const PageDataType*)pixelBoxDY.data + pageElementSize * (pixelBoxDY.rowPitch * (rowDY - pixelBoxDY.top) - pixelBoxDY.left);

			int columnPageIndex = column * pageElementSize;
			int columnDXPageIndex = columnDX * pageElementSize;

			int columnInnerEnd = std::min<int>(visibleQuadRect.right, std::min<int>(pixelBox.right - 1, availableDataRect.right));

			do
			{
				VertexDataType height = heightConverterFunctor((PageDataType)pixelRowData[columnPageIndex]);
				VertexDataType gradientX = heightConverterFunctor((PageDataType)pixelRowDXData[columnDXPageIndex]) - height;
				VertexDataType gradientY = heightConverterFunctor((PageDataType)pixelRowDYData[columnPageIndex]) - height;
