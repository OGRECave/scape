		...
		struct CachedBufferDescription
		{
			...
			inline bool operator==(const CachedBufferDescription &rhs) const
			{
				return	type == rhs.type &&
						quadColumnCount == rhs.quadColumnCount &&
						quadRowCount == rhs.quadRowCount &&
						spacingNorth == rhs.spacingNorth &&
						spacingEast == rhs.spacingEast &&
						spacingSouth == rhs.spacingSouth &&
...
)
{
	...
	CachedBufferDescription key = {type, quadColumnCount, quadRowCount, spacingNorth, spacingEast, spacingSouth, spacingWest};
	...
	if (bufferIt == mCachedBuffers->end())
	{
		...
		case Ogre::HardwareIndexBuffer::IT_16BIT:
			value.buffer = createBuffer< Ogre::uint16 >(type, quadColumnCount, quadRowCount, spacingNorth, spacingEast, spacingSouth, spacingWest);
			break;

		case Ogre::HardwareIndexBuffer::IT_32BIT:
			value.buffer = createBuffer< Ogre::uint32 >(type, quadColumnCount, quadRowCount, spacingNorth, spacingEast, spacingSouth, spacingWest);
			break;
		}
	...
	int spacingWest)
{
	...
	assert((quadColumnCount + 1) * (quadRowCount + 1) <= ((long long)1 << (sizeof(IndexType) << 3)) && _T("Index bitsize insufficient"));
