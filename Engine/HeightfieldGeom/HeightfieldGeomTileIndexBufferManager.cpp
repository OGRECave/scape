...
namespace ScapeEngine {
    namespace HeightfieldGeomTileIndexBufferManagerNS {
        ...
        struct CachedBufferDescription
        {
            ...
            {
                return type == rhs.type && quadColumnCount == rhs.quadColumnCount && quadRowCount == rhs.quadRowCount
                    && spacingNorth == rhs.spacingNorth && spacingEast == rhs.spacingEast
                    && spacingSouth == rhs.spacingSouth && spacingWest == rhs.spacingWest;
            }
...
// ----------------------------------------------------------------------------
HeightfieldGeomTileIndexBufferManager::~HeightfieldGeomTileIndexBufferManager()
{
    delete mCachedBuffers;
}

// ----------------------------------------------------------------------------
Ogre::HardwareIndexBufferSharedPtr HeightfieldGeomTileIndexBufferManager::getBuffer(
    int quadColumnCount, int quadRowCount, int spacingNorth, int spacingEast, int spacingSouth, int spacingWest)
{
    Ogre::HardwareIndexBuffer::IndexType type;
    ...
    CachedBufferDescription key
        = { type, quadColumnCount, quadRowCount, spacingNorth, spacingEast, spacingSouth, spacingWest };
    ...
    if (bufferIt == mCachedBuffers->end())
    {
        ...
        case Ogre::HardwareIndexBuffer::IT_16BIT:
            value.buffer = createBuffer<Ogre::uint16>(
                type, quadColumnCount, quadRowCount, spacingNorth, spacingEast, spacingSouth, spacingWest);
            break;

        case Ogre::HardwareIndexBuffer::IT_32BIT:
            value.buffer = createBuffer<Ogre::uint32>(
                type, quadColumnCount, quadRowCount, spacingNorth, spacingEast, spacingSouth, spacingWest);
            break;
        }
...
Ogre::HardwareIndexBufferSharedPtr HeightfieldGeomTileIndexBufferManager::createBuffer(
    Ogre::HardwareIndexBuffer::IndexType type, int quadColumnCount, int quadRowCount, int spacingNorth, int spacingEast,
    int spacingSouth, int spacingWest)
{
    size_t indexCount;
    ...
    Ogre::HardwareIndexBufferSharedPtr buffer = Ogre::HardwareBufferManager::getSingleton().createIndexBuffer(
