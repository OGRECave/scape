...
HeightfieldBuffer* HeightfieldOperationBrushBase::getTempHeightfieldBuffer()
{
    if (!mTempHeightfieldBuffer)
    {
        ...
        HeightfieldBufferSet* bufferSet = mInOutHeightfieldBuffer->getHeightfieldBufferSet();
        string tempName =
            mInOutHeightfieldBuffer->getName() + _T("Temp") + Ogre::StringConverter::toString((long)this);
        mTempHeightfieldBuffer =
            bufferSet->get(bufferSet->createFrom(tempName, mInOutHeightfieldBuffer->getHandle(), true));
    }
...
void HeightfieldOperationBrushBase::tickDo()
{
    ...
    if (mHeightfieldGeom && mBrush->getHeightfieldGeom() && !mBrush->isActive() &&
        getBrushInstanceCount() == 0 && isSchedulerDone())
    {
        finishDo();
