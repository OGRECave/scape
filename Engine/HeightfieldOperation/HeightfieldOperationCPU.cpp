...
HeightfieldOperationCPU::HeightfieldOperationCPU()
    ...
    , mBrushName(_T("HeightfieldOperationCPU"))
...
// ----------------------------------------------------------------------------
bool HeightfieldOperationCPU::isActive(EState state)
{
    return mIsStateActive && mState == state;
}

// ----------------------------------------------------------------------------
bool HeightfieldOperationCPU::isActive()
{
...
void HeightfieldOperationCPU::tickDo()
{
    ...
    if (mHeightfieldGeom && mBrushPathNodes.size() < 4 && !mBrush->isActive() && mBrush->getHeightfieldGeom())
    {
        finishDo();
        mIsStateActive = false;
        return;
