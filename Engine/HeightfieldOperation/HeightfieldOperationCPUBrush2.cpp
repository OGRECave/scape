...
public:
	// ----------------------------------------------------------------------------
	HeightfieldOperationTaskPageCPUBrush(HeightfieldOperationBrush* operation, const BrushPageCoords pageCoords, const BrushInstances& brushInstances) 
	...
	{
		HeightfieldBufferPage* inOutPage = mOperation->getInOutHeightfieldBuffer()->getPage(mPageCoords.first, mPageCoords.second, HeightfieldBuffer::PAGEACCESSMODE_READONLY);
		if (!inOutPage) return;
		...
		while (!mBrushInstances.empty())
		{
			...
			Ogre::Real sizeChangeScale = brushInstance.strength * mOperation->getStrength();// * 0.01f;//* changeSpeed;// * (1.0f - expf(-getEngineCore()->getTimeSinceLastFrame() / changeSpeed));
			...
			Ogre::Rect relativeRect = Utils::translatedRect(Utils::intersectTRect(newUpdateRect, pageRect), -pageRect.left, -pageRect.top);
			...
			//Ogre::Real heightscale = inOutPage->getHeightfieldBufferSet()->getHeightRangeMax() - inOutPage->getHeightfieldBufferSet()->getHeightRangeMin();
			...
			for (int x = (int)relativeRect.left; x < (int)relativeRect.right; ++x)
			{
				for (int z = (int)relativeRect.top; z < (int)relativeRect.bottom; ++z)
				{
					...
					void* pixelPtr = (Ogre::uint8*)pixelBox.data + Ogre::PixelUtil::getNumElemBytes(pixelBox.format) * (z*pageRect.width()+x);
...
:	HeightfieldOperationPolicyBrushInstancerSpacer<HeightfieldOperationCPUBrush2, HeightfieldOperationPolicySchedulerTaskPageBatch<HeightfieldOperationCPUBrush2, HeightfieldOperationBrushBase> >()
{
...
// ----------------------------------------------------------------------------
HeightfieldOperationTaskPage* HeightfieldOperationCPUBrush2::createTaskPage(const BrushPageCoords& pageCoords, const BrushInstances& brushInstances)
{
