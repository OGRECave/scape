...
void HeightfieldOperationCPUErosion::applyPrimary(const Ogre::Vector3& position, Ogre::Real strength)
{
    ...
    for (int it = 0; it < outerRadius - innerRadius - 1; ++it)
    {
        ...
        for (int z = 0; z < (int)editRect.height(); ++z)
        {
            for (int x = 0; x < (int)editRect.width(); ++x, index += 3)
            {
                ...
                // Ogre::Real dz = (Ogre::Real)z - cz;
                // Ogre::Real s = dx * dx + dz * dz;
                float water = Utils::max(0.0f, 1.0f
        ...
        for (int z = 1; z < (int)editRect.height() - 1; ++z)
        {
            for (int x = 1; x < (int)editRect.width() - 1; ++x, index += 3)
            {
                ...
                if (water > 0.0f)
                {
                    ...
                    // Ogre::Real steepscale = std::min(1.0f, 2000.0f * abs(dheight1 - dheight2) + abs(dheight3 -
                    // dheight4));

                    Ogre::Real blur
                        = (dheightavg - height) * mKA
                        * Utils::max(
                              0.0f, 1.0f - ((x - cx) * (x - cx) + (z - cz) * (z - cz)) / innerRadius / innerRadius);
                    ...
                    */
                    ...
                    Ogre::Real delta3 = 0.257f * std::max(0.0f, height + water - dataIn[index - dataRowPitch]
                                                         - dataIn[index - dataRowPitch + 1]);
                    Ogre::Real delta4 = 0.257f * std::max(0.0f, height + water - dataIn[index + dataRowPitch]
                                                         - dataIn[index + dataRowPitch + 1]);

                    Ogre::Real delta5 = 0.103f * std::max(0.0f, height + water - dataIn[index - dataRowPitch - 3]
                                                         - dataIn[index - dataRowPitch - 2]);
                    Ogre::Real delta6 = 0.103f * std::max(0.0f, height + water - dataIn[index - dataRowPitch + 3]
                                                         - dataIn[index - dataRowPitch + 4]);
                    Ogre::Real delta7 = 0.103f * std::max(0.0f, height + water - dataIn[index + dataRowPitch - 3]
                                                         - dataIn[index + dataRowPitch - 2]);
                    Ogre::Real delta8 = 0.103f * std::max(0.0f, height + water - dataIn[index + dataRowPitch + 3]
                                                         - dataIn[index + dataRowPitch + 4]);
                    ...
                    */
                    ...
                    // Ogre::Real dz = (Ogre::Real)z - cz;
                    // Ogre::Real s = dx * dx + dz * dz;
                    // Ogre::Real deltaWater = 0.25f * Utils::min(1.0f, water / deltaSum) * Utils::clamp(s * scale +
                    // bias, 0.0f, 1.0f);
                    //}
                }
            }
    ...
    for (int z = 0; z < (int)editRect.height(); ++z)
    {
        for (int x = 0; x < (int)editRect.width(); ++x, index += 3)
        {
            dataIn[index] = Utils::clamp(
                dataIn[index] + dataIn[index + 2], 0.0f, 1.0f); // - dataIn[index+1];// - dataIn[index+1];
        }
    }
