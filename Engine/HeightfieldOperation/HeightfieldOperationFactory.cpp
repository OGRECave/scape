...
{
    assert(mOperationDescMap.find(className) == mOperationDescMap.end() &&
           _T("HeightfieldOperation class already registered"));
...
StringList HeightfieldOperationFactory::getOperationNameList()
{
    ...
    for (operationDescMapIt = mOperationDescMap.begin(); operationDescMapIt != operationDescMapItEnd;
         ++operationDescMapIt)
    {
...
HeightfieldOperation* HeightfieldOperationFactory::create(const string& className)
{
    ...
    if (operationDescMapIt == mOperationDescMap.end())
    {
        OGRE_EXCEPT(Ogre::Exception::ERR_ITEM_NOT_FOUND,
                    _T("Tried to create unknown class type '") + className +
                        _T("'for HeightfieldOperationFactory"),
