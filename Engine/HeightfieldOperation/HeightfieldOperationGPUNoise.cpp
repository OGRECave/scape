...
string HeightfieldOperationGPUNoise::setUIElementPropertyValue(const string& elementName, const string& value)
{
    ...
    case PROPERTYID_NOISESTRETCHANGLE:
    {
        intValue = Utils::clamp(intValue, 0, 10000);
        mNoiseStretchAngle = intValue * 1.0f;
        outValue = Ogre::StringConverter::toString(intValue);
        break;
    }
    case PROPERTYID_NOISESTRETCHRATIO:
    {
        intValue = Utils::clamp(intValue, 1, 10000);
        mNoiseStretchRatio = intValue * 0.01f;
        outValue = Ogre::StringConverter::toString(intValue);
        break;
    }
    ...
    case PROPERTYID_NOISETYPE:
    {
        intValue = Utils::clamp(intValue, 0, 5);
        mNoiseType = intValue;
        outValue = Ogre::StringConverter::toString(intValue);
        break;
    }
    ...
    case PROPERTYID_DISTORTSTRETCHANGLE:
    {
        intValue = Utils::clamp(intValue, 0, 10000);
        mDistortStretchAngle = intValue * 1.0f;
        outValue = Ogre::StringConverter::toString(intValue);
        break;
    }
    case PROPERTYID_DISTORTSTRETCHRATIO:
    {
        intValue = Utils::clamp(intValue, 1, 10000);
        mDistortStretchRatio = intValue * 0.01f;
        outValue = Ogre::StringConverter::toString(intValue);
        break;
    }
...
HeightfieldOperationGPUNoise::HeightfieldOperationGPUNoise()
    : HeightfieldOperationPolicyBrushInstancerSpacer<HeightfieldOperationGPUNoise,
          HeightfieldOperationPolicySchedulerTaskPageBatch<HeightfieldOperationGPUNoise,
...
// ----------------------------------------------------------------------------
HeightfieldOperationTaskPage* HeightfieldOperationGPUNoise::createTaskPage(
    const BrushPageCoords& pageCoords, const BrushInstances& brushInstances)
{
    ...
    case 1:
        ...
        break;
    case 2:
        matName += _T("_BILLOWED");
        break;
    case 3:
        matName += _T("_IQ");
        break;
    case 4:
        matName += _T("_EROSION1");
        break;
    case 5:
        matName += _T("_EROSION2");
        break;
    default:
        assert(false && _T("unknown noise type"));
    }
    ...
    //	return new HeightfieldOperationTaskPageGPUBrush(_T("GPU2DOperationPencil"), _T(""), true, this, this,
    //pageCoords, brushInstances);
    return new HeightfieldOperationTaskPageGPUBrush(
        _T("GPU2DOperationBrush"), matName, 16, false, this, this, pageCoords, brushInstances);
...
void HeightfieldOperationGPUNoise::onSetupGPU2DOperationQuadCustom(GPU2DOperationQuadCustomPtr quadPtr)
{
    ...
    for (int index = 0; index < 16; ++index)
    {
        ...
        scale *= 0.5f
            * Utils::lerp(mDistortCoarseRough, mDistortFineRough, Utils::min(1.0f, index / distortHighestOctave));
    }
    ...
    quadPtr->setMaterialShaderConstant(
        _T("noiseTrans"),
        Ogre::Vector4(cos(DEGTORAD(mNoiseStretchAngle)) / mNoiseLargestFeat * sqrtf(mNoiseStretchRatio),
    ...
    quadPtr->setMaterialShaderConstant(
        _T("distortTrans"),
        Ogre::Vector4(cos(DEGTORAD(mDistortStretchAngle)) / mDistortLargestFeat * sqrtf(mDistortStretchRatio),
    ...
    quadPtr->setMaterialShaderConstant(
        _T("scales"), Ogre::Vector4(mDistortScaleFine, mDistortScaleCoarse, mNoiseBlend, mNoisePeakify));
