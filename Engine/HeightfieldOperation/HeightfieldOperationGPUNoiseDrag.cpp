...
IMPLEMENT_UIELEMENTCONTAINERSIMPLE_CLASS(HeightfieldOperationGPUNoiseDrag);

#define DEGTORAD(a) ((3.141592f / 180.0f)*(a))

// ----------------------------------------------------------------------------
class HeightfieldOperationGPUNoiseDragTask
{
protected:
...
// ----------------------------------------------------------------------------
string HeightfieldOperationGPUNoiseDrag::setUIElementPropertyValue(const string& elementName, const string& value) 
{
	...
	case PROPERTYID_NOISEALLOWUP: {intValue = Utils::clamp(intValue, 0, 100); mNoiseAllowUp = intValue * 0.01f; outValue = Ogre::StringConverter::toString(intValue); break;}
	case PROPERTYID_NOISEALLOWDOWN: {intValue = Utils::clamp(intValue, 0, 100); mNoiseAllowDown = intValue * 0.01f; outValue = Ogre::StringConverter::toString(intValue); break;}
	...
	case PROPERTYID_DIRECTIONSTRETCHANGLE: {intValue = Utils::clamp(intValue, 0, 90); mDirectionStretchAngle = intValue * 1.0f; outValue = Ogre::StringConverter::toString(intValue); break;}
	case PROPERTYID_DIRECTIONSTRETCHRATIO: {intValue = Utils::clamp(intValue, 1, 100); mDirectionStretchRatio = intValue * 0.01f; outValue = Ogre::StringConverter::toString(intValue); break;}
...
:	HeightfieldOperationPolicyBrushInstancerSpacer<HeightfieldOperationGPUNoiseDrag, HeightfieldOperationPolicySchedulerTaskPageBatch<HeightfieldOperationGPUNoiseDrag, HeightfieldOperationBrushBase> >()
{
...
// ----------------------------------------------------------------------------
HeightfieldOperationTaskPage* HeightfieldOperationGPUNoiseDrag::createTaskPage(const BrushPageCoords& pageCoords, const BrushInstances& brushInstances)
{
...
//	return new HeightfieldOperationTaskPageGPUBrush(_T("GPU2DOperationPencil"), _T(""), true, this, this, pageCoords, brushInstances);
	return new HeightfieldOperationTaskPageGPUBrush(_T("GPU2DOperationNoiseDrag"), _T(""), 8, true, this, this, pageCoords, brushInstances);
//	return new HeightfieldOperationTaskPageGPUBrush(_T("GPU2DOperationBrush"), matName, false, this, this, pageCoords, brushInstances);
}

// ----------------------------------------------------------------------------
//void HeightfieldOperationGPUNoiseDrag::onSetupGPU2DOperationQuadCustom(GPU2DOperationQuadCustomPtr quadPtr)
void HeightfieldOperationGPUNoiseDrag::onSetupGPU2DOperationQuadBrush(GPU2DOperationQuadBrushPtr quadPtr)
{
	...
	quadPtr->setMaterialShaderConstant(_T("noiseTrans"), Ogre::Vector4(
		1.0f / mNoiseLargestFeat, 1.0f / mDistortLargestFeat, 0.0f, 0.0f));

	quadPtr->setMaterialShaderConstant(_T("directionTrans"), Ogre::Vector4(
	...
	quadPtr->setMaterialShaderConstant(_T("scales"), Ogre::Vector4(mDistortScaleFine, mDistortScaleCoarse, 0.5f * mNoiseAllowUp, 0.5f * mNoiseAllowDown));
