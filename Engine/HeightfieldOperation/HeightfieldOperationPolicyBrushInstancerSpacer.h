 ...
 *
 * A HeightfieldOperationBrushInstancr implementation that tries to space instances evenly across a spline generated
 * from brush positions
 *
...
namespace ScapeEngine {
    ...
    protected:
        ...
        HeightfieldOperationPolicyBrushInstancerSpacer()
            : mIsActiveStroke(true), mWasActiveStroke(false), mHasLastBrushInstancePosition(false), mPathNextParam(0.0f)
        {
        }
        ...
        virtual void brushInstancerTick()
        {
            ...
            if (this->isPencilMode())
            {
                while (this->mBrushPathNodes.size() >= 4)
                {
                    ...
                    Ogre::Matrix4 splineVectors(node1.position.x, node2.position.x, node3.position.x, node4.position.x,
                        node1.position.y, node2.position.y, node3.position.y, node4.position.y, node1.position.z,
                        node2.position.z, node3.position.z, node4.position.z, node1.pressure, node2.pressure,
                        node3.pressure, node4.pressure);
                    ...
                    Ogre::Real distance = std::max(1.0f, subPoint1.distance(subPoint2) + subPoint2.distance(subPoint3)
                            + subPoint3.distance(subPoint4) + subPoint4.distance(subPoint5));

                    distance = std::max(0.1f, node2.position.distance(node3.position));
                    Ogre::Real stepSize
                        = std::max(1.0f, this->mPathSpacing * (brush->getOuterRadius() - brush->getInnerRadius()));
                    ...
                    if (mPathNextParam < distance)
                    {
                        ...
                        strength = 0.5f * stepStrength * interpolation.w * interpolation.w * this->getStrength()
