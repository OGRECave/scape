...
namespace ScapeEngine
{
	...
	protected:
		...
		:	ParentClass(),
			...
			mAllowMultipleBrushInstances(false)
		{}


		// ----------------------------------------------------------------------------
		virtual void allowMultipleBrushInstances(bool allow)
		{
			mAllowMultipleBrushInstances = allow;
		}
		...
		virtual void schedulerTick()
		{
			...
			while (getBrushInstanceCount() &&  mWorkingBrushInstancesCount < allowedWorkingBrushInstances)
			{
				...
				Ogre::Rect editRect(
					std::max(0,column-radius), 
					std::max(0,row-radius), 
					std::min(inOutHeightfieldBuffer->getHeightfieldBufferSet()->getElementColumnCount(), column + radius),
					std::min(inOutHeightfieldBuffer->getHeightfieldBufferSet()->getElementRowCount(), row + radius));

				if (editRect.width() > 0 && editRect.height() > 0)
				{
					Ogre::Rect pageRect(
						inOutHeightfieldBuffer->getPageColumnFromElement(editRect.left),
						inOutHeightfieldBuffer->getPageRowFromElement(editRect.top),
						inOutHeightfieldBuffer->getPageColumnFromElement(editRect.right - 1) + 1,
					...
					for (int pageColumn = pageRect.left; pageColumn < pageRect.right; ++pageColumn)
					{
						...
						{
							mPageBinnedBrushInstances[BrushPageCoords(pageColumn, pageRow)].push_back(brushInstance);
						}
					}
				}
			...
			while (gridIt != gridItEnd)
			{
				...
				HeightfieldOperationTaskPageList::iterator pageIt, pageItEnd = mHeightfieldOperationTaskPageList.end();
				...
				if (!foundPage)
				{
					...
					HeightfieldOperationTaskPage* operationPage = operation->createTaskPage(pageCoords, brushInstances);
			...
			// parallel page processing, without updating inout buffer. only used for disallowed multiplebrushinstances
			bool isDone = false;
			...
			// parallel page processing, without updating inout buffer from temp and updating geoms. when disallowed multiplebrushinstances,
