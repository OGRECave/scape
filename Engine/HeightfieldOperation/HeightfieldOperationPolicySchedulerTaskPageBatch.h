    ...
    protected:
        ...
        virtual void schedulerTick()
        {
            ...
            while (this->getBrushInstanceCount() && mWorkingBrushInstancesCount < allowedWorkingBrushInstances)
            {
                ...
                Ogre::Rect editRect(std::max(0, column - radius), std::max(0, row - radius),
            ...
            // parallel page processing, without updating inout buffer from temp and updating geoms. when disallowed
            ...
            if (mAllowMultipleBrushInstances || (!mAllowMultipleBrushInstances && isDone))
            {
                ...
                for (pageIt = mHeightfieldOperationTaskPageList.begin(); pageIt != pageItEnd;)
                {
                    ...
                    if (page.isCompleted())
                    {
                        this->getInOutHeightfieldBuffer()->swapPage(
                            page.getPageCoords().first, page.getPageCoords().second, this->getTempHeightfieldBuffer());
