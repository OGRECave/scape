...
// ----------------------------------------------------------------------------
HeightfieldOperationStack::~HeightfieldOperationStack()
{
    clearAll();
}
...
bool HeightfieldOperationStack::doOperation()
{
    if (mNewOperation && mNewOperation->canInitiate(HeightfieldOperation::STATE_DO))
    {
        ...
        assert(geom && geom->getHeightfieldBuffer() && _T("HeightfieldOperationStack::getNewOperation() : Needs a ")
                                                       _T("current heighttfieldbuffer to bind an operation to"));
        ...
        mOperationHistory[mLastOperationIndex]->initiate(HeightfieldOperation::STATE_DO, geom->getHeightfieldBuffer());
        return true;
    }
    return false;
}
...
bool HeightfieldOperationStack::undoOperation()
{
    for (int newIndex = mLastOperationIndex; newIndex >= 0; --newIndex)
    {
        if (getOperation(newIndex)->canInitiate(HeightfieldOperation::STATE_UNDO))
        {
            ...
            getOperation(newIndex)->initiate(HeightfieldOperation::STATE_UNDO, geom->getHeightfieldBuffer());
