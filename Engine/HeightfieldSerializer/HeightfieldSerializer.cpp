...
// ----------------------------------------------------------------------------
Utils::SerialMemoryBufferPtr HeightfieldSerializer::serialize(
    HeightfieldBuffer* inBuffer, Ogre::PixelFormat pixelFormat)
{
    assert(inBuffer);
    ...
    {
        for (int pageColumnIndex = 0; pageColumnIndex < inBuffer->getHeightfieldBufferSet()->getPageColumnCount();
             ++pageColumnIndex)
        {
            HeightfieldBufferPage* page
                = inBuffer->getPage(pageColumnIndex, pageRowIndex, HeightfieldBuffer::PAGEACCESSMODE_READONLY);
            Ogre::PixelBox pixelBox(inBuffer->getHeightfieldBufferSet()->getElementColumnCountPerPage(),
                inBuffer->getHeightfieldBufferSet()->getElementRowCountPerPage(), 1, pixelFormat, writer.getData());
...
// ----------------------------------------------------------------------------
bool HeightfieldSerializer::deserialize(Utils::SerialMemoryBufferConstPtr inSerial, HeightfieldBuffer* outBuffer,
    Ogre::PixelFormat pixelFormat, string* error)
{
    assert(outBuffer);
    ...
    {
        for (int pageColumnIndex = 0; pageColumnIndex < outBuffer->getHeightfieldBufferSet()->getPageColumnCount();
             ++pageColumnIndex)
        {
            HeightfieldBufferPage* page
                = outBuffer->getPage(pageColumnIndex, pageRowIndex, HeightfieldBuffer::PAGEACCESSMODE_WRITE_DISCARD);
            Ogre::PixelBox pixelBox(outBuffer->getHeightfieldBufferSet()->getElementColumnCountPerPage(),
                outBuffer->getHeightfieldBufferSet()->getElementRowCountPerPage(), 1, pixelFormat, reader.getData());
