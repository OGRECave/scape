...
// ----------------------------------------------------------------------------
void InputManager::clearButtonDefinitions() 
{
	mButtonDefinitions.clear();
}
...
{
	if (mInputPointer) mInputPointer->startNewFrame();

	mSomeButtonStateJustChanged = false;

	if (mNumButtonsPressed == 0 || mFocusedRenderViewId == -1)
	{
		mFocusedRenderViewId = getEngineCore()->getRenderViewManager()->getViewIdAtRootSpacePosition(getInputPointer()->getCurrentState().getWindow2DPoint());
	}
	...
	if (mInputListener) mInputListener->captureInput();
}
...
{
	if (mInputListener) return mInputListener->getAnalogInput(analogInputId);
	return 0;
}


// ----------------------------------------------------------------------------
int InputManager::getHighestPriorityPressed() const
{
	return mHighestPriorityPressed;
}


// ----------------------------------------------------------------------------
bool InputManager::hasSomeButtonStateJustChanged() const
{
	return mSomeButtonStateJustChanged;
}
...
{
	ButtonDefinitionXMLSerializer::importSceneFromResource(filename, Ogre::ResourceGroupManager::DEFAULT_RESOURCE_GROUP_NAME);
}
...
{
	if (mInputPointer) mInputPointer->update();
}
...
void InputManager::onDeviceButtonPressed(DeviceButtonId::EDeviceButtonId deviceButton)
{
	...
	for (ButtonDefinitions::iterator defIt = mButtonDefinitions.begin(); defIt != mButtonDefinitions.end(); ++defIt)
	{
		if (!mButtons[defIt->mButtonId]->mPressed)
		{
			bool pressed = true;
			for (ButtonDefinition::DeviceButtonList::const_iterator defDeviceButtonIt = defIt->mDeviceButtons.begin(); 
				defDeviceButtonIt != defIt->mDeviceButtons.end(); 
				++defDeviceButtonIt)
			{
