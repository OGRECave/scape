...
std::pair<RenderView*, Ogre::Vector2> InputPointer::PointerState::getRenderView2DPoint() const
{
	if (mIsFocusedRenderViewDirty)
	{
		...
		{
			mFocusedRenderView->fromRootToNormalizedLocalSpace(
				mWindow2DPoint, 
				mFocusedRenderView2DPoint);
		}
...
const Ogre::Vector3& InputPointer::PointerState::get3DPoint() const
{
	...
	if (mIs3DPointDirty)
	{
		if (mFocusedRenderView)
		{
			...
			{
				Ogre::Vector3 pointerDirection = renderView2DPoint.first->getCameraController()->getViewportPickRayDirection(Ogre::Vector3(renderView2DPoint.second.x, renderView2DPoint.second.y, 0)).toOgreVector3();
				...
				getEngineCore()->getHeightfieldGeomManager()->getClosestHeightfieldGeomRayIntersection(
					intersectionRay, 
					*mFocusedRenderView->getCamera()->getSceneManager(), 
					m3DPointGeom,
					m3DPoint);
			}
		}
...
void InputPointer::update()
{
	...
	mCurrentState = PointerState(
		mInputManager->getAnalogInput(AnalogInputId::POINTER_X), 
		mInputManager->getAnalogInput(AnalogInputId::POINTER_Y),
		pressure);
