...
// ----------------------------------------------------------------------------
InputPointer::InputPointer(InputManager* inputManager) : mInputManager(inputManager), mCurrentState(0, 0, -1.0f)
{
}
...
const Ogre::Vector3& InputPointer::PointerState::get3DPoint() const
{
    ...
    if (mIs3DPointDirty)
    {
        if (mFocusedRenderView)
        {
            ...
            {
                Ogre::Vector3 pointerDirection = renderView2DPoint.first->getCameraController()
                                                     ->getViewportPickRayDirection(Ogre::Vector3(
                                                         renderView2DPoint.second.x, renderView2DPoint.second.y, 0))
                                                     .toOgreVector3();
                ...
                getEngineCore()->getHeightfieldGeomManager()->getClosestHeightfieldGeomRayIntersection(
                    intersectionRay, *mFocusedRenderView->getCamera()->getSceneManager(), m3DPointGeom, m3DPoint);
            }
        }
        else
        {
