...
static void tryLoadWinTabLibrary()
{
	...
	{
		pWTInfo = (WTINFO)GetProcAddress(sWinTabLibrary.get(), _T("WTInfoA")); if (!pWTInfo) return;
		pWTQueueSizeSet = (WTQUEUESIZESET)GetProcAddress(sWinTabLibrary.get(), _T("WTQueueSizeSet")); if (!pWTQueueSizeSet) return;
		pWTOpen = (WTOPEN)GetProcAddress(sWinTabLibrary.get(), _T("WTOpenA")); if (!pWTOpen) return;
		pWTClose = (WTCLOSE)GetProcAddress(sWinTabLibrary.get(), _T("WTClose")); if (!pWTClose) return;
		pWTGet = (WTGET)GetProcAddress(sWinTabLibrary.get(), _T("WTGetA")); if (!pWTGet) return;
		pWTPacketsGet = (WTPACKETSGET)GetProcAddress(sWinTabLibrary.get(), _T("WTPacketsGet")); if (!pWTPacketsGet) return;
...
:	mConnected(false),
	...
	mCurrentCursorId((Ogre::uint32)-1),
	mContextHandle(NULL),
	mPressure(-1.0f),
	mPacketQueueSize(0)
{
	...
	if (sIsWinTabAvailable && !mConnected)
	{
		if (pWTInfo(0, 0, NULL)) {
			...
			if (success)
			{
				...
				{
					Ogre::LogManager::getSingleton().logMessage(_T("Error during tablet connection setup: Device context unavailable"));
				}
			}
			else
			{
				Ogre::LogManager::getSingleton().logMessage(_T("Error during tablet connection setup: Device interface unavailablr"));
			}
		}
	}
}
...
void TabletInputWin32::capture()
{
	if (mConnected)
	{
		...
		//std::cout << (int)mContextHandle << " " << (int)mPacketQueueSize << " " << (int)sPacketBuffer << std::endl;
		...
		for (int i = 0; i < nPackets; i++) {
			...
			{
				mPressure = ((Ogre::Real)packet.pkNormalPressure - mPressureValueMin) / (mPressureValueMax - mPressureValueMin);
