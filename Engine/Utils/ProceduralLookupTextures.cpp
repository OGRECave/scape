...
static Ogre::uint8 perlinPermTable[256] = { 151, 160, 137, 91, 90, 15, 131, 13, 201, 95, 96, 53, 194, 233, 7, 225, 140,
    36, 103, 30, 69, 142, 8, 99, 37, 240, 21, 10, 23, 190, 6, 148, 247, 120, 234, 75, 0, 26, 197, 62, 94, 252, 219, 203,
    ...
    54, 65, 25, 63, 161, 255, 216, 80, 73, 209, 76, 132, 187, 208, 89, 18, 169, 200, 196, 135, 130, 116, 188, 159, 86,
...
static Ogre::uint8 perlinGradTable[16] = { 245, 176, 176, 245, 79, 245, 10, 176, 10, 79, 79, 10, 176, 10, 245, 79 };

// ----------------------------------------------------------------------------
void Utils::createPerlinPerm2DTexture(const string& textureName)
{
    Ogre::LogManager::getSingleton().logMessage(_T("Creating PerlinPerm2DTexture '") + textureName + _T("'..."));
    ...
    for (int y = 0; y < 256; ++y)
    {
        for (int x = 0; x < 256; ++x)
        {
            ...
            *ptr++ = (perlinPermTable[a] << 24) + (perlinPermTable[(a + 1) & 255] << 16) + (perlinPermTable[b] << 8)
...
{
    Ogre::LogManager::getSingleton().logMessage(_T("Creating PerlinGrad2DTexture '") + textureName + _T("'..."));
    ...
    for (int y = 0; y < 256; ++y)
    {
        for (int x = 0; x < 256; ++x)
        {
            ...
            *ptr++ = (perlinGradTable[((px & 7) << 1)] << 24) + (perlinGradTable[((px & 7) << 1) + 1] << 16)
