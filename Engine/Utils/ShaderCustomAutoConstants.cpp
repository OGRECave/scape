...
// ----------------------------------------------------------------------------
ShaderCustomAutoConstants::~ShaderCustomAutoConstants()
{
	clear();
}
...
// ----------------------------------------------------------------------------
bool ShaderCustomAutoConstants::updateShaderParameter(Ogre::GpuProgramParameters* params, const Ogre::GpuProgramParameters::AutoConstantEntry& autoConstantEntry) const
{
	if (autoConstantEntry.data < mAutoConstants.size())
	{
		...
		if (constant.guid && constant.guid != mGlobalActiveAutoConstantGUIDs[autoConstantEntry.data])
		{
			...
			{
				params->_writeRawConstants(autoConstantEntry.physicalIndex, constant.data, constant.dataCount);
			}
		}
	}
	return false;
...
// ----------------------------------------------------------------------------
void ShaderCustomAutoConstants::setConstant(size_t constantIndex, const Ogre::Real* data, int dataCount, bool isMatrix)
{
