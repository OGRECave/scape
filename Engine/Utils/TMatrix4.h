...
namespace ScapeEngine
{
    ...
    public:
		...
		inline TMatrix4<T>(const TVector4<T>& _col0, const TVector4<T>& _col1, const TVector4<T>& _col2, const TVector4<T>& _col3) :
		...
		inline TMatrix4<T>(const Ogre::Matrix4 &mtx) :
			...
			col2(mtx[0][2], mtx[1][2], mtx[2][2], mtx[3][2]),
			col3(mtx[0][3], mtx[1][3], mtx[2][3], mtx[3][3])
		{
        }
		...
		{
			return Ogre::Matrix4 (
				static_cast<Ogre::Real>(col0.x), 
				static_cast<Ogre::Real>(col1.x),
				static_cast<Ogre::Real>(col2.x),
				static_cast<Ogre::Real>(col3.x),

				static_cast<Ogre::Real>(col0.y),
				static_cast<Ogre::Real>(col1.y), 
				static_cast<Ogre::Real>(col2.y), 
				static_cast<Ogre::Real>(col3.y),

				static_cast<Ogre::Real>(col0.z), 
				static_cast<Ogre::Real>(col1.z), 
				static_cast<Ogre::Real>(col2.z), 
				static_cast<Ogre::Real>(col3.z),

				static_cast<Ogre::Real>(col0.w), 
				static_cast<Ogre::Real>(col1.w), 
				static_cast<Ogre::Real>(col2.w), 
				static_cast<Ogre::Real>(col3.w)
			);
        }
		...
		inline TVector3<T> operator *(const TVector3<T> &v) const
        {
			...
			return TVector3<T>
			(
				(col0.x * v.x + col1.x * v.y + col2.x * v.z + col3.x) * fInvW,
		...
		inline void fromEulerAngles(const TVector3<T>& eulerAngles, TVector3<T> translation = TVector3<T>::ZERO)
		{
			...
			TMatrix4<T> yawMat(
				TVector4<T>(cosy,0,-siny,0),
				TVector4<T>(0,1,0,0),
				TVector4<T>(siny,0,cosy,0),
				TVector4<T>::UNIT_W
				);

			TMatrix4<T> pitchMat(
				TVector4<T>(1,0,0,0),
				TVector4<T>(0,cosp,sinp,0),
				TVector4<T>(0,-sinp,cosp,0),
				TVector4<T>::UNIT_W
				);
	...
	template<class T> const TMatrix4<T> TMatrix4<T>::IDENTITY
	(
		TVector4<T>(1, 0, 0, 0),
		TVector4<T>(0, 1, 0, 0),
		TVector4<T>(0, 0, 1, 0),
	...
	template<class T> const TMatrix4<T> TMatrix4<T>::ZERO
	(
		TVector4<T>(0, 0, 0, 0),
		TVector4<T>(0, 0, 0, 0),
		TVector4<T>(0, 0, 0, 0),
