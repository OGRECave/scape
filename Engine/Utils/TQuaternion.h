...
public:
    ...
    // ----------------------------------------------------------------------------
    TQuaternion<T> operator+(const TQuaternion<T>& v) const
    {
        return TQuaternion<T>(x + v.x, y + v.y, z + v.z, w + v.w);
    }
    ...
    // ----------------------------------------------------------------------------
    friend TQuaternion<T> operator*(const double s, const TQuaternion<T>& v)
    {
        return TQuaternion<T>(v.x * s, v.y * s, v.z * s, v.w * s);
    }

    // ----------------------------------------------------------------------------
    TQuaternion<T> operator*(const TQuaternion<T>& q) const
    {
        return TQuaternion<T>(w * q.x + x * q.w + y * q.z - z * q.y, w * q.y + y * q.w + z * q.x - x * q.z,
    ...
    TQuaternion<T> fromEulerAngles(const TVector3<T> &eulerAngles)
    {
            ...
            {
                    TQuaternion<T> quat = TQuaternion<T>::fromAngleAxisPair(eulerAngles.y,
    TVector3<T>::AXIS_Y);
                    result = quat;
            }
            if (!fpAlmostZero(eulerAngles.x))
            {
                    TQuaternion<T> quat = TQuaternion<T>::fromAngleAxisPair(eulerAngles.x,
    TVector3<T>::AXIS_X);
                    if (fpAlmostEqual(result.w, 1)) result = quat; else result = result * quat;
            }
            if (!fpAlmostZero(eulerAngles.z))
            {
                    TQuaternion<T> quat = TQuaternion<T>::fromAngleAxisPair(eulerAngles.z,
    TVector3<T>::AXIS_Z);
                    if (fpAlmostEqual(result.w, 1)) result = quat; else result = result * quat;
            }
            return result;
    ...
    */
    ...
    inline TMatrix4<T> toMatrix4(const TVector3<T> translation = TVector3<T>::ZERO) const
    {
        ...
        return TMatrix4<T>(DVector4(1.0 - twoYY - twoZZ, twoXY + twoZW, twoXZ - twoYW, 0.0),
