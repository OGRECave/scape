...
const Utils::MooreNeighborhood::EDirection Utils::MooreNeighborhood::mOppositeDirection[]
    = { DIRECTION_S, DIRECTION_SW, DIRECTION_W, DIRECTION_NW, DIRECTION_N, DIRECTION_NE, DIRECTION_E, DIRECTION_SE };
...
Ogre::Vector4 Utils::getCatmullRomeSplineWeights(Ogre::Real param)
{
    ...
    return Ogre::Vector4(-0.166666667f * param3 + 0.500000000f * param2 - 0.333333333f * param,
        ...
        -0.500000000f * param3 + 0.500000000f * param2 + param, 0.166666667f * param3 - 0.166666667f * param);
}

// ----------------------------------------------------------------------------
Ogre::PixelBox Utils::clipPixelBox(
    const Ogre::PixelBox& pixelBox, const Ogre::Rect& relativeRect, int offsetX, int offsetY)
{
    ...
    for (Ogre::ushort i = 0; i < technique->getNumPasses(); ++i)
    {
        ...
        if (pass->hasFragmentProgram())
        {
            ...
            if (definition)
            {
                ...
                params->_setRawAutoConstant(physicalIndex, Ogre::GpuProgramParameters::ACT_CUSTOM, constantIndex,
                    Ogre::GPV_PER_OBJECT, definition->elementSize);
    ...
    if (resourcePtr.isNull())
    {
        ...
        if (!image.getSize())
        {
            Ogre::LogManager::getSingleton().logMessage(_T("Failed to load internal texture '" + fileName
                + _T("'. Loading midgray texture for resource'") + textureName + _T("' instead.")));
