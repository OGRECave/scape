...
Ogre::Vector4 Utils::getCatmullRomeSplineWeights(Ogre::Real param)
{
	...
	return Ogre::Vector4(
		-0.166666667f * param3 + 0.500000000f * param2 - 0.333333333f * param,
...
// ----------------------------------------------------------------------------
Ogre::PixelBox Utils::clipPixelBox(const Ogre::PixelBox& pixelBox, const Ogre::Rect& relativeRect, int offsetX, int offsetY)
{
	...
	int elementOffset = (relativeRect.top - pixelBox.top) * pixelBox.rowPitch + (relativeRect.left - pixelBox.left);
...
// ----------------------------------------------------------------------------
bool Utils::bindShaderCustomAutoConstant(Ogre::Technique* technique, size_t constantIndex, const string& constantName)
{
	...
	for (Ogre::ushort i = 0; i < technique->getNumPasses(); ++i)
    {
        ...
        if (pass->hasFragmentProgram())
        {
            ...
            while (constantIt.hasMoreElements())
            {
                ...
                if (constantEntry.paramType == Ogre::GpuProgramParameters::ACT_CUSTOM && constantEntry.data == constantIndex)
                {
					return true;
                }
            }
			const Ogre::GpuConstantDefinition* definition = params->_findNamedConstantDefinition(constantName, false);
			if (definition)
			{
				//GDC port
				//before: params->_setRawAutoConstant(definition->physicalIndex, Ogre::GpuProgramParameters::ACT_CUSTOM, constantIndex, definition->elementSize);
				size_t physicalIndex = params->_getFloatConstantPhysicalIndex(definition->logicalIndex, definition->elementSize, Ogre::GPV_PER_OBJECT);
				params->_setRawAutoConstant(physicalIndex, Ogre::GpuProgramParameters::ACT_CUSTOM, constantIndex, Ogre::GPV_PER_OBJECT, definition->elementSize);
...
// ----------------------------------------------------------------------------
Ogre::uint32 Utils::randHash(int in)
{
	return ((in * 1103515245 + 12345) >> 16) & 0x7FFF;
}
...
// ----------------------------------------------------------------------------
bool Utils::getTextureFromInternalFile(const string& textureName, const string& group, const string& fileName)
{
	...
	if (resourcePtr.isNull())
	{
		...
		try
		{
			...
			Ogre::TexturePtr texturePtr = Ogre::TextureManager::getSingleton().loadImage(textureName, group, image);
			texturePtr->load();
			successful = true;
		}
		catch (...) {}
		...
		if (!image.getSize())
		{
			Ogre::LogManager::getSingleton().logMessage(_T("Failed to load internal texture '" + fileName + _T("'. Loading midgray texture for resource'") + textureName + _T("' instead.")));
...
// ----------------------------------------------------------------------------
bool Utils::getTextureFromExternalFile(const string& textureName, const string& group, const string& fileName)
{
	Ogre::ResourcePtr resourcePtr = Ogre::TextureManager::getSingleton().getByName(textureName);
	bool successful = true;
	...
	if (resourcePtr.isNull())
	{
		...
		if (!successful)
		{
			Ogre::LogManager::getSingleton().logMessage(_T("Failed to load external texture '" + fileName + _T("'. Loading midgray texture for resource'") + textureName + _T("' instead.")));
			image.load(_T("midgrey.png"), Ogre::ResourceGroupManager::DEFAULT_RESOURCE_GROUP_NAME);
			Ogre::TextureManager::getSingleton().loadImage(textureName, group, image);
		}
	}
...
string Utils::getRelativePath(const string& absolutePath, const string& basePath)
{
	...
	string givenBasePathCmp = givenBasePath; Ogre::StringUtil::toLowerCase(givenBasePathCmp);
	string absoluteBasePathCmp = absoluteBasePath; Ogre::StringUtil::toLowerCase(absoluteBasePathCmp);
